...
exports.EventsMixin = {
    ...
    escape(evt) {
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.ESCAPE, { event: evt }));
    },
    click(me) {
        const evt = me.getEvent();
        let cell = me.getCell();
        const mxe = new EventObject_js_1.default(InternalEvent_js_1.default.CLICK, { event: evt, cell });
        if (me.isConsumed()) {
            mxe.consume();
        }
        this.fireEvent(mxe);
        if (this.isEnabled() && !(0, EventUtils_js_1.isConsumed)(evt) && !mxe.isConsumed()) {
            if (cell) {
                ...
                if (cell != null && (!this.isToggleEvent(evt) || !(0, EventUtils_js_1.isAltDown)(evt))) {
                    let temp = cell;
                    let swimlanes = [];
                    while (temp != null) {
                        temp = temp.getParent();
    ...
    dblClick(evt, cell = null) {
        const mxe = new EventObject_js_1.default(InternalEvent_js_1.default.DOUBLE_CLICK, { event: evt, cell });
        this.fireEvent(mxe);
    ...
    tapAndHold(me) {
        const evt = me.getEvent();
        const mxe = new EventObject_js_1.default(InternalEvent_js_1.default.TAP_AND_HOLD, {
            event: evt,
            cell: me.getCell(),
        });
        ...
        if (this.isEnabled() &&
            ...
            if (cell) {
                ...
                if (state) {
                    ...
                    connectionHandler.first = new Point_js_1.default(me.getGraphX(), me.getGraphY());
                    connectionHandler.edgeState = connectionHandler.createEdgeState(me);
                    connectionHandler.previous = state;
                    connectionHandler.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.START, { state: connectionHandler.previous }));
                }
            }
        }
    },
    ...
    updateMouseEvent(me, evtName) {
        const pt = (0, styleUtils_js_1.convertPoint)(this.getContainer(), me.getX(), me.getY());
        me.graphX = pt.x - this.getPanDx();
        me.graphY = pt.y - this.getPanDy();
        // Searches for rectangles using method if native hit detection is disabled on shape
        if (!me.getCell() && this.isMouseDown && evtName === InternalEvent_js_1.default.MOUSE_MOVE) {
            const cell = this.getCellAt(pt.x, pt.y, null, true, true, (state) => {
                return (!state.shape ||
                    ...
                    state.shape.fill !== Constants_js_1.NONE);
            });
    ...
    getStateForTouchEvent(evt) {
        ...
        const pt = (0, styleUtils_js_1.convertPoint)(this.getContainer(), x, y);
        const cell = this.getCellAt(pt.x, pt.y);
        return cell ? this.getView().getState(cell) : null;
    },
    isEventIgnored(evtName, me, sender) {
        const mouseEvent = (0, EventUtils_js_1.isMouseEvent)(me.getEvent());
        let result = false;
        ...
        if (eventSource && evtName !== InternalEvent_js_1.default.MOUSE_MOVE) {
            InternalEvent_js_1.default.removeGestureListeners(eventSource, null, this.mouseMoveRedirect, this.mouseUpRedirect);
            this.mouseMoveRedirect = null;
            this.mouseUpRedirect = null;
            this.setEventSource(null);
        }
        else if (!Client_js_1.default.IS_GC && eventSource && me.getSource() !== eventSource) {
            result = true;
        }
        else if (eventSource &&
            Client_js_1.default.IS_TOUCH &&
            evtName === InternalEvent_js_1.default.MOUSE_DOWN &&
            !mouseEvent &&
            !(0, EventUtils_js_1.isPenEvent)(me.getEvent())) {
            this.setEventSource(me.getSource());
            this.mouseMoveRedirect = (evt) => {
                this.fireMouseEvent(InternalEvent_js_1.default.MOUSE_MOVE, new InternalMouseEvent_js_1.default(evt, this.getStateForTouchEvent(evt)));
            };
            this.mouseUpRedirect = (evt) => {
                this.fireMouseEvent(InternalEvent_js_1.default.MOUSE_UP, new InternalMouseEvent_js_1.default(evt, this.getStateForTouchEvent(evt)));
            };
            InternalEvent_js_1.default.addGestureListeners(eventSource, null, this.mouseMoveRedirect, this.mouseUpRedirect);
        }
        ...
        if (!(0, EventUtils_js_1.isPopupTrigger)(this.lastEvent) &&
            evtName !== InternalEvent_js_1.default.MOUSE_MOVE &&
            this.lastEvent.detail === 2) {
            return true;
        }
        // Filters out of sequence events or mixed event types during a gesture
        if (evtName === InternalEvent_js_1.default.MOUSE_UP && this.isMouseDown) {
            this.isMouseDown = false;
        }
        else if (evtName === InternalEvent_js_1.default.MOUSE_DOWN && !this.isMouseDown) {
        ...
        // and mouse events that are not in sync with the current internal button state
            ...
            (((!Client_js_1.default.IS_FF || evtName !== InternalEvent_js_1.default.MOUSE_MOVE) &&
                this.isMouseDown &&
                this.isMouseTrigger !== mouseEvent) ||
                (evtName === InternalEvent_js_1.default.MOUSE_DOWN && this.isMouseDown) ||
                (evtName === InternalEvent_js_1.default.MOUSE_UP && !this.isMouseDown))) {
            result = true;
        }
        if (!result && evtName === InternalEvent_js_1.default.MOUSE_DOWN) {
    ...
    isSyntheticEventIgnored(evtName, me, sender) {
        ...
        if (this.ignoreMouseEvents && mouseEvent && evtName !== InternalEvent_js_1.default.MOUSE_MOVE) {
            this.ignoreMouseEvents = evtName !== InternalEvent_js_1.default.MOUSE_UP;
            result = true;
        }
        else if (Client_js_1.default.IS_FF && !mouseEvent && evtName === InternalEvent_js_1.default.MOUSE_UP) {
    ...
    isEventSourceIgnored(evtName, me) {
        ...
        const candidate = !(0, EventUtils_js_1.isMouseEvent)(me.getEvent()) || (0, EventUtils_js_1.isLeftMouseButton)(me.getEvent());
        return (evtName === InternalEvent_js_1.default.MOUSE_DOWN &&
            candidate &&
            (name === 'select' ||
                name === 'option' ||
                (name === 'input' &&
    ...
    fireMouseEvent(evtName, me, sender) {
        ...
        if ((!this.nativeDblClickEnabled && !(0, EventUtils_js_1.isPopupTrigger)(me.getEvent())) ||
            (this.doubleTapEnabled &&
                Client_js_1.default.IS_TOUCH &&
                ((0, EventUtils_js_1.isTouchEvent)(me.getEvent()) || (0, EventUtils_js_1.isPenEvent)(me.getEvent())))) {
            ...
            if (evtName === InternalEvent_js_1.default.MOUSE_DOWN) {
                if (this.lastTouchEvent &&
                    ...
                    if (evtName === InternalEvent_js_1.default.MOUSE_UP) {
            ...
            else if ((this.isMouseDown || evtName === InternalEvent_js_1.default.MOUSE_UP) &&
                ...
                const valid = cell ||
                    (((0, EventUtils_js_1.isTouchEvent)(me.getEvent()) || (0, EventUtils_js_1.isPenEvent)(me.getEvent())) &&
                        (Client_js_1.default.IS_GC || Client_js_1.default.IS_SF));
                if (valid &&
                    Math.abs(this.lastTouchX - me.getX()) < this.doubleTapTolerance &&
                    Math.abs(this.lastTouchY - me.getY()) < this.doubleTapTolerance) {
                    this.dblClick(me.getEvent(), cell);
        ...
        if (!this.isEventIgnored(evtName, me, sender)) {
            ...
            this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.FIRE_MOUSE_EVENT, { eventName: evtName, event: me }));
            if (Client_js_1.default.IS_SF || Client_js_1.default.IS_GC || me.getEvent().target !== this.getContainer()) {
                const container = this.getContainer();
                if (evtName === InternalEvent_js_1.default.MOUSE_MOVE &&
                    this.isMouseDown &&
                    this.isAutoScroll() &&
                    !(0, EventUtils_js_1.isMultiTouchEvent)(me.getEvent())) {
                    this.scrollPointToVisible(me.getGraphX(), me.getGraphY(), this.isAutoExtend());
                }
                else if (evtName === InternalEvent_js_1.default.MOUSE_UP &&
                ...
                for (const l of mouseListeners) {
                    ...
                    else if (evtName === InternalEvent_js_1.default.MOUSE_MOVE) {
            ...
            if (((0, EventUtils_js_1.isTouchEvent)(me.getEvent()) || (0, EventUtils_js_1.isPenEvent)(me.getEvent())) &&
                evtName === InternalEvent_js_1.default.MOUSE_DOWN &&
    ...
    consumeMouseEvent(evtName, me, sender) {
        ...
        if (evtName === InternalEvent_js_1.default.MOUSE_DOWN && (0, EventUtils_js_1.isTouchEvent)(me.getEvent())) {
            me.consume(false);
        }
    },
    fireGestureEvent(evt, cell = null) {
        ...
        this.lastTouchTime = 0;
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.GESTURE, { event: evt, cell }));
    },
    sizeDidChange() {
        ...
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.SIZE, { bounds }));
    },
    isCloneEvent(evt) {
        return (0, EventUtils_js_1.isControlDown)(evt);
    },
    isTransparentClickEvent(evt) {
        return false;
    },
    isToggleEvent(evt) {
        return Client_js_1.default.IS_MAC ? (0, EventUtils_js_1.isMetaDown)(evt) : (0, EventUtils_js_1.isControlDown)(evt);
    },
    isGridEnabledEvent(evt) {
        return !(0, EventUtils_js_1.isAltDown)(evt);
    },
    ...
    getPointForEvent(evt, addOffset = true) {
        const p = (0, styleUtils_js_1.convertPoint)(this.getContainer(), (0, EventUtils_js_1.getClientX)(evt), (0, EventUtils_js_1.getClientY)(evt));
        const s = this.getView().scale;
        const tr = this.getView().translate;
