...
exports.EllipsePerimeter = void 0;
...
const EllipsePerimeter = (bounds, _vertex, next, orthogonal = false) => {
    ...
    if (dx === 0 && dy !== 0) {
        return new Point_js_1.default(cx, cy + (b * dy) / Math.abs(dy));
    }
    if (dx === 0 && dy === 0) {
        return new Point_js_1.default(px, py);
    }
    if (orthogonal) {
        if (py >= y && py <= y + bounds.height) {
            ...
            return new Point_js_1.default(cx + tx, py);
        }
        if (px >= x && px <= x + bounds.width) {
            ...
            return new Point_js_1.default(px, cy + ty);
        }
    }
    ...
    return new Point_js_1.default(xout, yout);
};
