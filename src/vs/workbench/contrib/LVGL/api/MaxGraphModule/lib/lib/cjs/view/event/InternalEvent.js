...
const InternalMouseEvent_js_1 = __importDefault(require("./InternalMouseEvent.js"));
...
class InternalEvent {
    ...
    static addGestureListeners(node, startListener = null, moveListener = null, endListener = null) {
        if (startListener) {
            InternalEvent.addListener(node, Client_js_1.default.IS_POINTER ? 'pointerdown' : 'mousedown', startListener);
        }
        if (moveListener) {
            InternalEvent.addListener(node, Client_js_1.default.IS_POINTER ? 'pointermove' : 'mousemove', moveListener);
        }
        if (endListener) {
            InternalEvent.addListener(node, Client_js_1.default.IS_POINTER ? 'pointerup' : 'mouseup', endListener);
        }
        if (!Client_js_1.default.IS_POINTER && Client_js_1.default.IS_TOUCH) {
            if (startListener) {
    ...
    static removeGestureListeners(node, startListener, moveListener, endListener) {
        if (startListener) {
            InternalEvent.removeListener(node, Client_js_1.default.IS_POINTER ? 'pointerdown' : 'mousedown', startListener);
        }
        if (moveListener) {
            InternalEvent.removeListener(node, Client_js_1.default.IS_POINTER ? 'pointermove' : 'mousemove', moveListener);
        }
        if (endListener) {
            InternalEvent.removeListener(node, Client_js_1.default.IS_POINTER ? 'pointerup' : 'mouseup', endListener);
        }
        if (!Client_js_1.default.IS_POINTER && Client_js_1.default.IS_TOUCH) {
            if (startListener) {
    ...
    static addMouseWheelListener(funct, target) {
        if (funct != null) {
            ...
            if (Client_js_1.default.IS_SF && !Client_js_1.default.IS_TOUCH) {
                let scale = 1;
                ...
                InternalEvent.addGestureListeners(target, ((evt) => {
                    if (!(0, EventUtils_js_1.isMouseEvent)(evt) && evt.pointerId != null) {
                        evtCache.push(evt);
                    }
                }), ((evt) => {
                    if (!(0, EventUtils_js_1.isMouseEvent)(evt) && evtCache.length == 2) {
