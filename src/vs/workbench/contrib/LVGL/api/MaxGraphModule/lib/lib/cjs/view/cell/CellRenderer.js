...
const ConnectorShape_js_1 = __importDefault(require("../shape/edge/ConnectorShape.js"));
...
const ShapeRegistry_js_1 = require("../shape/ShapeRegistry.js");
const StencilShapeRegistry_js_1 = require("../shape/stencil/StencilShapeRegistry.js");
...
const Shape_js_1 = __importDefault(require("../shape/Shape.js"));
...
class CellRenderer {
    constructor() {
         ...
         */
        this.defaultEdgeShape = ConnectorShape_js_1.default;
        /**
         * Defines the default shape for vertices.
         * @default {@link RectangleShape}.
         */
        this.defaultVertexShape = RectangleShape_js_1.default;
        /**
         * Defines the default shape for labels.
         * @default {@link TextShape}.
         */
        this.defaultTextShape = TextShape_js_1.default;
        /**
    ...
    createShape(state) {
        ...
        const stencil = StencilShapeRegistry_js_1.StencilShapeRegistry.get(state.style.shape);
        if (stencil) {
            return new Shape_js_1.default(stencil);
        }
     ...
     */
    getShape(name) {
        return ShapeRegistry_js_1.ShapeRegistry.get(name);
    }
    /**
    ...
    configureShape(state) {
        ...
        if (shape) {
            ...
            shape.indicatorColor = state.getIndicatorColor() || Constants_js_1.NONE;
            shape.indicatorStrokeColor = state.style.indicatorStrokeColor || Constants_js_1.NONE;
            shape.indicatorGradientColor = state.getIndicatorGradientColor() || Constants_js_1.NONE;
    ...
    installListeners(state) {
        ...
        if (state.shape) {
            InternalEvent_js_1.default.addGestureListeners(state.shape.node, (evt) => {
            ...
            // Uses double click timeout in mxGraph for quirks mode
            if (graph.isNativeDblClickEnabled()) {
                InternalEvent_js_1.default.addListener(state.shape.node, 'dblclick', (evt) => {
                    if (this.isShapeEvent(state, evt)) {
                        graph.dblClick(evt, state.cell);
                        InternalEvent_js_1.default.consume(evt);
                    }
    ...
    redrawLabel(state, forced) {
        ...
        const isForceHtml = graph.isHtmlLabel(state.cell) || (value && (0, domUtils_js_1.isNode)(value));
        ...
        if (state.text == null && value != null && ((0, domUtils_js_1.isNode)(value) || value.length > 0)) {
    ...
    getLabelBounds(state) {
        ...
        let bounds = new Rectangle_js_1.default(state.absoluteOffset.x, state.absoluteOffset.y);
        if (isEdge) {
            // @ts-ignore
    ...
    getControlBounds(state, w, h) {
        if (state.control != null) {
            ...
            return state.cell.isEdge()
                ? new Rectangle_js_1.default(Math.round(cx - (w / 2) * s), Math.round(cy - (h / 2) * s), Math.round(w * s), Math.round(h * s))
                : new Rectangle_js_1.default(Math.round(cx - (w / 2) * s), Math.round(cy - (h / 2) * s), Math.round(w * s), Math.round(h * s));
        }
        return null;
    }
    ...
    isShapeInvalid(state, shape) {
        return (shape.bounds == null ||
            ...
            (state.absolutePoints.length > 0 &&
                !(0, arrayUtils_js_1.equalPoints)(shape.points, state.absolutePoints)));
    }
    /**
