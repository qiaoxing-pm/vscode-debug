...
exports.ConnectionsMixin = void 0;
...
exports.ConnectionsMixin = {
    ...
    getOutlineConstraint(point, terminalState, me) {
        if (terminalState.shape) {
            ...
            const alpha = (0, mathUtils_js_1.toRadians)(terminalState.shape.getShapeRotation());
            if (alpha !== 0) {
                const cos = Math.cos(-alpha);
                const sin = Math.sin(-alpha);
                const ct = new Point_js_1.default(bounds.getCenterX(), bounds.getCenterY());
                point = (0, mathUtils_js_1.getRotatedPoint)(point, cos, sin, ct);
            }
            let sx = 1;
            let sy = 1;
            let dx = 0;
            ...
            point = new Point_js_1.default((point.x - bounds.x) * sx - dx + bounds.x, (point.y - bounds.y) * sy - dy + bounds.y);
            ...
            return new ConnectionConstraint_js_1.default(new Point_js_1.default(x, y), false);
        }
        return null;
    },
    ...
    getConnectionConstraint(edge, terminal, source = false) {
        ...
        if (x !== undefined) {
            ...
            if (y !== undefined) {
                point = new Point_js_1.default(x, y);
            }
        }
        let perimeter = false;
        let dx = 0;
        ...
        return new ConnectionConstraint_js_1.default(point, perimeter, null, dx, dy);
    },
    ...
    getConnectionPoint(vertex, constraint, round = true) {
        ...
        if (constraint.point) {
            ...
            const cx = new Point_js_1.default(bounds.getCenterX(), bounds.getCenterY());
            ...
            point = new Point_js_1.default(bounds.x + constraint.point.x * bounds.width + constraint.dx * scale, bounds.y + constraint.point.y * bounds.height + constraint.dy * scale);
            ...
            if (constraint.perimeter) {
                if (r1 !== 0) {
                    ...
                    point = (0, mathUtils_js_1.getRotatedPoint)(point, cos, sin, cx);
                }
            ...
            if (r2 !== 0 && point) {
                const rad = (0, mathUtils_js_1.toRadians)(r2);
    ...
    connectCell(edge, terminal = null, source = false, constraint = null) {
        this.batchUpdate(() => {
            ...
            this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.CONNECT_CELL, 'edge', edge, 'terminal', terminal, 'source', source, 'previous', previous));
        });
        return edge;
    },
    cellConnected(edge, terminal, source = false, constraint = null) {
        this.batchUpdate(() => {
            ...
            this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.CELL_CONNECTED, 'edge', edge, 'terminal', terminal, 'source', source, 'previous', previous));
        });
    },
                    ...
                    if (geo) {
                        ...
                        if (state && pstate) {
                            ...
                            if (src && this.isCellDisconnectable(cell, src, true)) {
                                ...
                                if (!src && pts[0]) {
                                    geo.setTerminalPoint(new Point_js_1.default(pts[0].x / scale - tr.x + dx, pts[0].y / scale - tr.y + dy), true);
                            ...
                            if (trg && this.isCellDisconnectable(cell, trg, false)) {
                                ...
                                if (!trg) {
                                    ...
                                    const p = pts[n];
                                    if (p) {
                                        geo.setTerminalPoint(new Point_js_1.default(p.x / scale - tr.x + dx, p.y / scale - tr.y + dy), false);
