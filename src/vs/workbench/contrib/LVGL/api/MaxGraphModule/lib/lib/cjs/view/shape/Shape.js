...
class Shape {
    constructor(stencil = null) {
        ...
        this.startSize = 1;
        this.endSize = 1;
        this.startArrow = Constants_js_1.NONE;
        this.endArrow = Constants_js_1.NONE;
        this.direction = 'east';
        this.flipH = false;
        this.flipV = false;
        this.isShadow = false;
        ...
        this.image = null;
        this.imageSrc = null;
        this.indicatorColor = Constants_js_1.NONE;
        this.indicatorStrokeColor = Constants_js_1.NONE;
        this.indicatorGradientColor = Constants_js_1.NONE;
    ...
    getSvgScreenOffset() {
        const sw = this.stencil && this.stencil.strokeWidthValue !== 'inherit'
            ...
            : (this.strokeWidth ?? 0);
        return (0, mathUtils_js_1.mod)(Math.max(1, Math.round(sw * this.scale)), 2) === 1 ? 0.5 : 0;
    }
    /**
     ...
     */
    create() {
        return document.createElementNS(Constants_js_1.NS_SVG, 'g');
    }
    redraw() {
    ...
    updateBoundsFromPoints() {
        ...
        if (pts.length > 0 && pts[0]) {
            this.bounds = new Rectangle_js_1.default(Math.round(pts[0].x), Math.round(pts[0].y), 1, 1);
            for (const pt of pts) {
                if (pt) {
                    this.bounds.add(new Rectangle_js_1.default(Math.round(pt.x), Math.round(pt.y), 1, 1));
                }
            }
        }
    }
    ...
    getLabelBounds(rect) {
        ...
        if (labelMargins) {
            ...
            return (0, mathUtils_js_1.getDirectedBounds)(rect, labelMargins, this.style, flipH, flipV);
        }
        return rect;
    }
    /**
    ...
    createSvgCanvas() {
        if (!this.node)
            return null;
        const canvas = new SvgCanvas2D_js_1.default(this.node, false);
    ...
    destroyCanvas(canvas) {
        ...
        if (canvas instanceof SvgCanvas2D_js_1.default) {
    ...
    paint(c) {
        ...
        if (bounds) {
            ...
            if (this.stencil) {
                ...
                if (this.points.length > 0) {
                    ...
                    for (let i = 0; i < this.points.length; i += 1) {
                        ...
                        if (p) {
                            pts.push(new Point_js_1.default(p.x / s, p.y / s));
                        }
                    }
            ...
            if (bg && c.state && !(0, utils_js_1.isNullish)(c.state.transform)) {
    ...
    configureCanvas(c, x, y, w, h) {
        ...
        if (this.fill !== Constants_js_1.NONE && this.gradient !== Constants_js_1.NONE) {
     ...
     */
    getBaseArcSize() {
        return (this.style?.arcSize ?? Constants_js_1.LINE_ARCSIZE) / 2;
    }
    /**
    ...
    addPoints(c, pts, rounded = false, arcSize, close = false, exclude = [], initialMove = true) {
        if (pts.length > 0) {
            ...
            if (close && rounded) {
                pts = pts.slice();
                const p0 = pts[0];
                const wp = new Point_js_1.default(pe.x + (p0.x - pe.x) / 2, pe.y + (p0.y - pe.y) / 2);
                pts.splice(0, 0, wp);
            }
            let pt = pts[0];
            let i = 1;
            ...
            while (i < (close ? pts.length : pts.length - 1)) {
                let tmp = pts[(0, mathUtils_js_1.mod)(i, pts.length)];
                let dx = pt.x - tmp.x;
                let dy = pt.y - tmp.y;
                if (rounded && (dx !== 0 || dy !== 0) && exclude.indexOf(i - 1) < 0) {
                    ...
                    let next = pts[(0, mathUtils_js_1.mod)(i + 1, pts.length)];
                    // Uses next non-overlapping point
                    while (i < pts.length - 2 &&
                        Math.round(next.x - tmp.x) === 0 &&
                        Math.round(next.y - tmp.y) === 0) {
                        next = pts[(0, mathUtils_js_1.mod)(i + 2, pts.length)];
                        i++;
                    }
                    dx = next.x - tmp.x;
                    dy = next.y - tmp.y;
                    ...
                    tmp = new Point_js_1.default(x2, y2);
                }
                else {
    ...
    updateBoundingBox() {
        ...
        if (this.bounds) {
            ...
            if (bbox) {
                ...
                if (rot !== 0) {
                    bbox = (0, mathUtils_js_1.getBoundingBox)(bbox, rot);
                }
            }
    ...
    augmentBoundingBox(bbox) {
        if (this.isShadow) {
            bbox.width += Math.ceil(config_js_1.StyleDefaultsConfig.shadowOffsetX * this.scale);
            bbox.height += Math.ceil(config_js_1.StyleDefaultsConfig.shadowOffsetX * this.scale);
        }
        // Adds strokeWidth
    ...
    createTransparentSvgRectangle(x, y, w, h) {
        const rect = document.createElementNS(Constants_js_1.NS_SVG, 'rect');
        rect.setAttribute('x', String(x));
        rect.setAttribute('y', String(y));
        rect.setAttribute('width', String(w));
        rect.setAttribute('height', String(h));
        rect.setAttribute('fill', Constants_js_1.NONE);
        rect.setAttribute('stroke', Constants_js_1.NONE);
    ...
    setTransparentBackgroundImage(node) {
        node.style.backgroundImage = `url('${Client_js_1.default.imageBasePath}/transparent.gif')`;
    }
    /**
     * Paints the line shape.
     */
     ...
     */
    destroy() {
        InternalEvent_js_1.default.release(this.node);
        if (this.node.parentNode) {
