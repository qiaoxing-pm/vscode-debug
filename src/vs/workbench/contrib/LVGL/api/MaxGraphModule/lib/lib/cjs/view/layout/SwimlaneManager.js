 ...
 * @category Layout
 */
class SwimlaneManager extends EventSource_js_1.default {
    ...
    setGraph(graph) {
        ...
        this.graph = graph;
        if (this.graph) {
            this.graph.addListener(InternalEvent_js_1.default.ADD_CELLS, this.addHandler);
            this.graph.addListener(InternalEvent_js_1.default.CELLS_RESIZED, this.resizeHandler);
        }
    }
    /**
            ...
            this.graph.batchUpdate(() => {
                ...
                for (const cell of cells) {
                    if (!this.isSwimlaneIgnored(cell)) {
                        ...
                        if (geo) {
                            const size = new Rectangle_js_1.default(0, 0, geo.width, geo.height);
                            let top = cell;
                            let current = top;
                            while (current) {
                                top = current;
...
exports.default = SwimlaneManager;
