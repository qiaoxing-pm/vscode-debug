...
class ConstraintHandler {
    constructor(graph) {
         ...
         */
        this.pointImage = new ImageBox_js_1.default(`${Client_js_1.default.imageBasePath}/point.gif`, 5, 5);
         ...
         */
        this.highlightColor = Constants_js_1.DEFAULT_VALID_COLOR;
        ...
        this.graph.model.addListener(InternalEvent_js_1.default.CHANGE, this.resetHandler);
        this.graph.view.addListener(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, this.resetHandler);
        this.graph.view.addListener(InternalEvent_js_1.default.TRANSLATE, this.resetHandler);
        this.graph.view.addListener(InternalEvent_js_1.default.SCALE, this.resetHandler);
        this.graph.addListener(InternalEvent_js_1.default.ROOT, this.resetHandler);
    }
    /**
     ...
     */
    isKeepFocusEvent(me) {
        return (0, EventUtils_js_1.isShiftDown)(me.getEvent());
    }
    /**
    ...
    update(me, source, existingEdge, point) {
        if (this.isEnabled() && !this.isEventIgnored(me)) {
            ...
            if (!this.mouseleaveHandler && this.graph.container) {
                ...
                InternalEvent_js_1.default.addListener(this.graph.container, 'mouseleave', this.resetHandler);
            }
            const tol = this.getTolerance(me);
            const x = point ? point.x : me.getGraphX();
            const y = point ? point.y : me.getGraphY();
            const grid = new Rectangle_js_1.default(x - tol, y - tol, 2 * tol, 2 * tol);
            const mouse = new Rectangle_js_1.default(me.getGraphX() - tol, me.getGraphY() - tol, 2 * tol, 2 * tol);
            ...
            if (!this.isKeepFocusEvent(me) &&
                (!this.currentFocusArea ||
                    ...
                    !(0, mathUtils_js_1.intersects)(this.currentFocusArea, mouse)) &&
            ...
            if (this.focusIcons.length > 0 &&
                ...
                for (let i = 0; i < this.focusIcons.length; i += 1) {
                    ...
                    if ((this.intersects(this.focusIcons[i], mouse, source, existingEdge) ||
                        ...
                        tmp.grow(Constants_js_1.HIGHLIGHT_SIZE + 1);
                        tmp.width -= 1;
                        tmp.height -= 1;
                        if (!this.focusHighlight) {
                            ...
                            InternalEvent_js_1.default.redirectMouseEvents(hl.node, this.graph, getState);
                        }
    ...
    redraw() {
        if (this.currentFocus && this.constraints && this.focusIcons.length > 0) {
            ...
            this.currentFocusArea = new Rectangle_js_1.default(state.x, state.y, state.width, state.height);
            for (let i = 0; i < this.constraints.length; i += 1) {
                ...
                const bounds = new Rectangle_js_1.default(Math.round(cp.x - img.width / 2), Math.round(cp.y - img.height / 2), img.width, img.height);
    ...
    setFocus(me, state, source) {
        ...
        if (this.constraints && state) {
            ...
            for (let i = 0; i < this.constraints.length; i += 1) {
                ...
                const { src } = img;
                const bounds = new Rectangle_js_1.default(Math.round(cp.x - img.width / 2), Math.round(cp.y - img.height / 2), img.width, img.height);
                const icon = new ImageShape_js_1.default(bounds, src);
                ...
                icon.redraw();
                InternalEvent_js_1.default.redirectMouseEvents(icon.node, this.graph, getState);
    ...
    createHighlightShape() {
        const hl = new RectangleShape_js_1.default(new Rectangle_js_1.default(), this.highlightColor, this.highlightColor, Constants_js_1.HIGHLIGHT_STROKEWIDTH);
        hl.opacity = Constants_js_1.HIGHLIGHT_OPACITY;
        return hl;
    }
    /**
    ...
    intersects(icon, rectangle, source, existingEdge) {
        return (0, mathUtils_js_1.intersects)(icon.bounds, rectangle);
    }
    /**
    ...
    onDestroy() {
        ...
        if (this.mouseleaveHandler && this.graph.container) {
            InternalEvent_js_1.default.removeListener(this.graph.container, 'mouseleave', this.mouseleaveHandler);
            this.mouseleaveHandler = null;
        }
    }
}
exports.default = ConstraintHandler;
