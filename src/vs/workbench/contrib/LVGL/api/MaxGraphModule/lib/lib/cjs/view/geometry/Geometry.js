 ...
 * @category Geometry
 */
class Geometry extends Rectangle_js_1.default {
    ...
    swap() {
        if (this.alternateBounds) {
            const old = new Rectangle_js_1.default(this.x, this.y, this.width, this.height);
    ...
    rotate(angle, cx) {
        ...
        if (!this.relative) {
            const ct = new Point_js_1.default(this.getCenterX(), this.getCenterY());
        ...
        if (this.sourcePoint) {
            const pt = (0, mathUtils_js_1.getRotatedPoint)(this.sourcePoint, cos, sin, cx);
            this.sourcePoint.x = Math.round(pt.x);
            this.sourcePoint.y = Math.round(pt.y);
        }
        // Translates the target point
        if (this.targetPoint) {
            const pt = (0, mathUtils_js_1.getRotatedPoint)(this.targetPoint, cos, sin, cx);
        ...
        if (this.points) {
            for (let i = 0; i < this.points.length; i += 1) {
                if (this.points[i]) {
                    const pt = (0, mathUtils_js_1.getRotatedPoint)(this.points[i], cos, sin, cx);
    ...
    equals(geom) {
        ...
        return (super.equals(geom) &&
            ...
            (0, arrayUtils_js_1.equalPoints)(this.points, geom.points) &&
    ...
    clone() {
        return (0, cloneUtils_js_1.clone)(this);
    }
}
