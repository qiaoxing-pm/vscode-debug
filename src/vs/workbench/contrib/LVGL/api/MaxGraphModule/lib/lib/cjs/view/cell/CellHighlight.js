...
class CellHighlight {
    constructor(graph, highlightColor, strokeWidth, dashed) {
        ...
        this.spacing = 2;
        this.graph = graph;
        this.highlightColor = highlightColor ?? Constants_js_1.DEFAULT_VALID_COLOR;
        this.strokeWidth = strokeWidth ?? Constants_js_1.HIGHLIGHT_STROKEWIDTH;
        this.dashed = dashed ?? false;
        this.opacity = Constants_js_1.HIGHLIGHT_OPACITY;
        ...
        this.graph.getView().addListener(InternalEvent_js_1.default.SCALE, this.repaintHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.TRANSLATE, this.repaintHandler);
        this.graph
            .getView()
            .addListener(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, this.repaintHandler);
        this.graph.getDataModel().addListener(InternalEvent_js_1.default.CHANGE, this.repaintHandler);
        // Hides the marker if the current root changes
        this.resetHandler = () => {
            this.hide();
        };
        this.graph.getView().addListener(InternalEvent_js_1.default.DOWN, this.resetHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.UP, this.resetHandler);
    }
    /**
    ...
    repaint() {
        if (this.state && this.shape) {
            ...
            if (this.state.cell.isEdge()) {
                ...
                this.shape.bounds = new Rectangle_js_1.default(this.state.x - this.spacing, this.state.y - this.spacing, this.state.width + 2 * this.spacing, this.state.height + 2 * this.spacing);
