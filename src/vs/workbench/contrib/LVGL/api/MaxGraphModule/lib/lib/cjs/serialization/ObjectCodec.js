...
class ObjectCodec {
    ...
    isExcluded(obj, attr, value, write) {
        return attr == ObjectIdentity_js_1.default.FIELD_NAME || this.exclude.indexOf(attr) >= 0;
    }
    /**
    ...
    encodeObject(enc, obj, node) {
        ...
        for (const i in obj) {
            ...
            if (value != null && !this.isExcluded(obj, name, value, true)) {
                if ((0, mathUtils_js_1.isInteger)(name)) {
                    name = null;
                }
    ...
    encodeValue(enc, obj, name, value, node) {
        if (value != null) {
            if (this.isReference(obj, name, value, true)) {
                ...
                if (tmp == null) {
                    config_js_1.GlobalConfig.logger.warn(`ObjectCodec.encode: No ID for ${this.getName()}.${name}=${value}`);
                    return; // exit
                }
                value = tmp;
            }
    ...
    writeComplexAttribute(enc, obj, name, value, node) {
        ...
        if (child != null) {
            ...
            config_js_1.GlobalConfig.logger.warn(`ObjectCodec.encode: No node for ${this.getName()}.${name}: ${value}`);
        }
    }
    /**
    ...
    isNumericAttribute(dec, attr, obj) {
        ...
        (obj instanceof Geometry_js_1.default && geometryNumericAttributes.includes(attr.name)) ||
            (obj instanceof Point_js_1.default && pointNumericAttributes.includes(attr.name)) ||
            (0, mathUtils_js_1.isNumeric)(attr.value));
    }
    /**
    ...
    decodeAttribute(dec, attr, obj) {
        if (!this.isIgnoredAttribute(dec, attr, obj)) {
            ...
            if (this.isReference(obj, fieldname, value, false)) {
                ...
                if (tmp == null) {
                    config_js_1.GlobalConfig.logger.warn(`ObjectCodec.decode: No object for ${this.getName()}.${name}=${value}`);
                    return; // exit
                }
                value = tmp;
            }
    ...
    decodeChildren(dec, node, obj) {
        ...
        while (child) {
            ...
            if ((0, utils_js_1.isElement)(child) && !this.processInclude(dec, child, obj)) {
        ...
        if (fieldname == null || !this.isExcluded(obj, fieldname, child, false)) {
            ...
            if (child.nodeName === 'add') {
                ...
                if (value == null && ObjectCodec.allowEval) {
                    value = (0, utils_js_1.doEval)((0, domUtils_js_1.getTextContent)(child));
                }
            }
            else {
    ...
    processInclude(dec, node, into) {
        if (node.nodeName === 'include') {
            ...
            if (name != null) {
                try {
                    const xml = (0, requestUtils_js_1.load)(name).getDocumentElement();
                    if (xml != null) {
                        dec.decode(xml, into);
                    }
                }
