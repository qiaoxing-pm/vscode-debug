...
class Codec {
    ...
    getId(obj) {
        ...
        if (obj != null) {
            ...
            if (id == null && obj instanceof Cell_js_1.default) {
                id = obj.getId();
                if (id == null) {
                    // Uses an on-the-fly Id
                    id = CellPath_js_1.default.create(obj);
                    if (id.length === 0) {
                        id = 'root';
                    }
                }
    ...
    encode(obj) {
        ...
        if (obj != null && obj.constructor != null) {
            const enc = CodecRegistry_js_1.default.getCodec(obj.constructor);
            if (enc != null) {
                node = enc.encode(this, obj);
            }
            else if ((0, domUtils_js_1.isNode)(obj)) {
                node = (0, domUtils_js_1.importNode)(this.document, obj, true);
            }
            else {
                config_js_1.GlobalConfig.logger.warn(`Codec.encode: No codec for ${(0, StringUtils_js_1.getFunctionName)(obj.constructor)}`);
            }
        }
        return node;
    }
    ...
    decode(node, into) {
        ...
        let obj = null;
        if ((0, utils_js_1.isElement)(node)) {
            const dec = CodecRegistry_js_1.default.getCodecByName(node.nodeName);
            if (dec != null) {
                obj = dec.decode(this, node, into);
            }
            else {
                config_js_1.GlobalConfig.logger.warn(`Codec.decode: No codec found for node '${node.nodeName}', so the node won't be decoded, the original XML Element is returned instead.`);
    ...
    decodeCell(node, restoreStructures = true) {
        ...
        let decoder = CodecRegistry_js_1.default.getCodec(node.nodeName);
        ...
        if (!this.isCellCodec(decoder)) {
            ...
            while (child != null && !this.isCellCodec(decoder)) {
                decoder = CodecRegistry_js_1.default.getCodec(child.nodeName);
                child = child.nextSibling;
            }
        }
        if (!this.isCellCodec(decoder)) {
            decoder = CodecRegistry_js_1.default.getCodec(Cell_js_1.default);
        }
