...
const ElbowEdgeHandler_js_1 = __importDefault(require("./ElbowEdgeHandler.js"));
const config_js_1 = require("./config.js");
class EdgeSegmentHandler extends ElbowEdgeHandler_js_1.default {
    constructor(state) {
        super(state);
        this.points = [];
    }
    ...
    getCurrentPoints() {
        ...
        if ((pts.length === 2 && pts[0] && pts[1]) ||
            ...
            pts = [pts[0], new Point_js_1.default(cx, cy), new Point_js_1.default(cx, cy), pts[pts.length - 1]];
        }
        return pts;
    }
    /**
    ...
    getPreviewPoints(point) {
        ...
        if (result.length === 1) {
            ...
            if ((source != null && (0, mathUtils_js_1.contains)(source, x, y)) ||
                (target != null && (0, mathUtils_js_1.contains)(target, x, y))) {
                result = [point, point];
            }
        }
        return result;
    ...
    updatePreviewState(edge, point, terminalState, me) {
        ...
        if (!this.isSource && !this.isTarget) {
            ...
            // Handles special case of transitions from straight vertical to routed
                ...
                result = [new Point_js_1.default(point.x, y0), new Point_js_1.default(point.x, ye)];
            }
            this.points = result;
    ...
    start(x, y, index) {
        ...
        if (this.bends != null &&
            ...
            !this.isSource &&
            !this.isTarget) {
            (0, styleUtils_js_1.setOpacity)(this.bends[index].node, 100);
        }
    }
    /**
     * Adds custom bends for the center of each segment.
            ...
            if (pts != null && pts.length > 1) {
                ...
                if (pts.length === 4 &&
                    ...
                    if (Math.round(pts[0].y - pts[pts.length - 1].y) === 0) {
                        const cx = pts[0].x + (pts[pts.length - 1].x - pts[0].x) / 2;
                        pts[1] = new Point_js_1.default(cx, pts[1].y);
                        pts[2] = new Point_js_1.default(cx, pts[2].y);
                    }
                    else {
                        const cy = pts[0].y + (pts[pts.length - 1].y - pts[0].y) / 2;
                        pts[1] = new Point_js_1.default(pts[1].x, cy);
                        pts[2] = new Point_js_1.default(pts[2].x, cy);
                    }
                }
                for (let i = 0; i < pts.length - 1; i += 1) {
                    if (this.bends[i + 1] != null) {
                        p0 = pts[i];
                        pe = pts[i + 1];
                        const pt = new Point_js_1.default(p0.x + (pe.x - p0.x) / 2, p0.y + (pe.y - p0.y) / 2);
                        const b = this.bends[i + 1].bounds;
                        this.bends[i + 1].bounds = new Rectangle_js_1.default(Math.floor(pt.x - b.width / 2), Math.floor(pt.y - b.height / 2), b.width, b.height);
                ...
                if (straight) {
                    (0, styleUtils_js_1.setOpacity)(this.bends[1].node, config_js_1.EdgeHandlerConfig.virtualBendOpacity);
                    (0, styleUtils_js_1.setOpacity)(this.bends[3].node, config_js_1.EdgeHandlerConfig.virtualBendOpacity);
                }
            }
        }
    }
}
exports.default = EdgeSegmentHandler;
