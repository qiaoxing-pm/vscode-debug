...
const InternalMouseEvent_js_1 = __importDefault(require("./event/InternalMouseEvent.js"));
const CurrentRootChange_js_1 = __importDefault(require("./undoable_changes/CurrentRootChange.js"));
const EventUtils_js_1 = require("../util/EventUtils.js");
const cloneUtils_js_1 = require("../util/cloneUtils.js");
const EdgeStyleRegistry_js_1 = require("./style/edge/EdgeStyleRegistry.js");
const PerimeterRegistry_js_1 = require("./style/perimeter/PerimeterRegistry.js");
 ...
 * respectively.
 */
class GraphView extends EventSource_js_1.default {
    constructor(graph) {
        super();
        // TODO: Document me!
         ...
         */
        this.updatingDocumentResource = (0, i18n_utils_js_1.isI18nEnabled)() ? 'updatingDocument' : '';
        /**
        ...
        this.graphBounds = new Rectangle_js_1.default();
        this.scale = 1;
        /**
         * {@link Point} that specifies the current translation. Default is a new
         * empty {@link Point}.
         */
        this.translate = new Point_js_1.default();
        this.states = new Map();
        /**
    ...
    setScale(value) {
        ...
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.SCALE, { scale: value, previousScale }));
    }
    /**
     ...
     */
    setTranslate(dx, dy) {
        const previousTranslate = new Point_js_1.default(this.translate.x, this.translate.y);
        ...
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.TRANSLATE, {
        ...
        if (cells.length > 0) {
            for (let i = 0; i < cells.length; i += 1) {
                if (cells[i].isVertex() || cells[i].isEdge()) {
                    ...
                    if (state) {
                        if (!result) {
                            result = Rectangle_js_1.default.fromRectangle(state);
                        }
                        else {
                            result.add(state);
                        }
    ...
    setCurrentRoot(root) {
        if (this.currentRoot !== root) {
            const change = new CurrentRootChange_js_1.default(this, root);
            change.execute();
            const edit = new UndoableEdit_js_1.default(this, true);
            edit.add(change);
    ...
    scaleAndTranslate(scale, dx, dy) {
        ...
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, {
            scale,
            previousScale,
            translate: this.translate,
            previousTranslate: previousTranslate,
     ...
     */
    validate(cell = null) {
        const t0 = config_js_1.GlobalConfig.logger.enter('GraphView.validate');
        ...
        config_js_1.GlobalConfig.logger.leave('GraphView.validate', t0);
    }
    /**
     ...
     */
    getEmptyBounds() {
        return new Rectangle_js_1.default(this.translate.x * this.scale, this.translate.y * this.scale);
    }
    /**
    ...
    createBackgroundPageShape(bounds) {
        return new RectangleShape_js_1.default(bounds, 'white', 'black');
    }
    /**
    ...
    validateBackgroundImage() {
        ...
        if (bg) {
            if (!this.backgroundImage || this.backgroundImage.imageSrc !== bg.src) {
                ...
                const bounds = new Rectangle_js_1.default(0, 0, 1, 1);
                this.backgroundImage = new ImageShape_js_1.default(bounds, bg.src);
            ...
            if (this.backgroundPageShape == null) {
                ...
                if (this.backgroundPageShape.node) {
                    ...
                    if (graph.isNativeDblClickEnabled()) {
                        InternalEvent_js_1.default.addListener(this.backgroundPageShape.node, 'dblclick', ((evt) => {
                            graph.dblClick(evt);
                        }));
                    }
                    // Adds basic listeners for graph event dispatching outside the
                    // container and finishing the handling of a single gesture
                    InternalEvent_js_1.default.addGestureListeners(this.backgroundPageShape.node, (evt) => {
                        graph.fireMouseEvent(InternalEvent_js_1.default.MOUSE_DOWN, new InternalMouseEvent_js_1.default(evt));
                    }, (evt) => {
                        ...
                        if (graph.isMouseDown && !(0, EventUtils_js_1.isConsumed)(evt)) {
                            graph.fireMouseEvent(InternalEvent_js_1.default.MOUSE_MOVE, new InternalMouseEvent_js_1.default(evt));
                        }
                    }, (evt) => {
                        graph.fireMouseEvent(InternalEvent_js_1.default.MOUSE_UP, new InternalMouseEvent_js_1.default(evt));
                    });
                }
            }
            else {
    ...
    getBackgroundPageBounds() {
        ...
        return new Rectangle_js_1.default(this.scale * this.translate.x, this.scale * this.translate.y, fmt.width * ps, fmt.height * ps);
    }
    /**
    ...
    updateVertexState(state, geo) {
        ...
        if (geo.relative && pState && !pState.cell.isEdge()) {
            const alpha = (0, mathUtils_js_1.toRadians)(pState.style.rotation ?? 0);
            if (alpha !== 0) {
                const cos = Math.cos(alpha);
                const sin = Math.sin(alpha);
                const ct = new Point_js_1.default(state.getCenterX(), state.getCenterY());
                const cx = new Point_js_1.default(pState.getCenterX(), pState.getCenterY());
                const pt = (0, mathUtils_js_1.getRotatedPoint)(ct, cos, sin, cx);
    ...
    getFixedTerminalPoint(edge, terminal, source, constraint) {
        ...
        if (!pt && !terminal) {
            ...
            if (pt) {
                pt = new Point_js_1.default(s * (tr.x + pt.x + orig.x), s * (tr.y + pt.y + orig.y));
            }
        }
        return pt;
    }
    ...
    updateBoundsFromStencil(state) {
        ...
        if (state &&
            ...
            previous = Rectangle_js_1.default.fromRectangle(state);
    ...
    updatePoints(edge, points, source, target) {
        ...
        if (edgeStyle && source) {
            ...
            for (let i = 0; i < points.length; i += 1) {
                if (points[i]) {
                    const pt = (0, cloneUtils_js_1.clone)(points[i]);
    ...
    transformControlPoint(state, pt, ignoreScale = false) {
        if (state && pt) {
            ...
            return new Point_js_1.default(scale * (pt.x + this.translate.x + orig.x), scale * (pt.y + this.translate.y + orig.y));
        }
        return null;
    }
    /**
    ...
    getEdgeStyle(edge, points = [], source = null, target = null) {
        ...
        if (typeof edgeStyle === 'string') {
            let tmp = EdgeStyleRegistry_js_1.EdgeStyleRegistry.get(edgeStyle);
            if (!tmp && this.isAllowEval()) {
                tmp = (0, utils_js_1.doEval)(edgeStyle);
            }
            edgeStyle = tmp;
        }
    ...
    getFloatingTerminalPoint(edge, start, end, source) {
        ...
        const alpha = (0, mathUtils_js_1.toRadians)(start.style.rotation ?? 0);
        const center = new Point_js_1.default(start.getCenterX(), start.getCenterY());
        if (alpha !== 0) {
            const cos = Math.cos(-alpha);
            const sin = Math.sin(-alpha);
            next = (0, mathUtils_js_1.getRotatedPoint)(next, cos, sin, center);
        }
        ...
        if (pt && alpha !== 0) {
            const cos = Math.cos(alpha);
            const sin = Math.sin(alpha);
            pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, center);
        }
        return pt;
    }
    /**
        ...
        if (terminal != null) {
            ...
            if (perimeter != null && next != null) {
                ...
                if (bounds.width > 0 || bounds.height > 0) {
                    point = new Point_js_1.default(next.x, next.y);
                    let flipH = false;
                    let flipV = false;
    ...
    getPerimeterFunction(state) {
        ...
        if (typeof perimeter === 'string') {
            let tmp = PerimeterRegistry_js_1.PerimeterRegistry.get(perimeter);
            if (tmp == null && this.isAllowEval()) {
                tmp = (0, utils_js_1.doEval)(perimeter);
            }
            perimeter = tmp;
        }
    ...
    getNextPoint(edge, opposite, source = false) {
        ...
        if (!point && opposite) {
            point = new Point_js_1.default(opposite.getCenterX(), opposite.getCenterY());
        }
    ...
    getRelativePoint(edgeState, x, y) {
        ...
        if (geometry) {
            ...
            if (geometry.relative && pointCount > 1) {
                ...
                let minDist = (0, mathUtils_js_1.ptSegDistSq)(p0.x, p0.y, pe.x, pe.y, x, y);
                let length = 0;
                let index = 0;
                let tmp = 0;
                for (let i = 2; i < pointCount; i += 1) {
                    p0 = pe;
                    pe = absolutePoints[i];
                    const dist = (0, mathUtils_js_1.ptSegDistSq)(p0.x, p0.y, pe.x, pe.y, x, y);
                    tmp += segments[i - 2];
                    if (dist <= minDist) {
                        minDist = dist;
                        index = i - 1;
                ...
                let yDistance = Math.sqrt((0, mathUtils_js_1.ptSegDistSq)(p0.x, p0.y, pe.x, pe.y, x, y));
                const direction = (0, mathUtils_js_1.relativeCcw)(p0.x, p0.y, pe.x, pe.y, x, y);
                if (direction === -1) {
                    yDistance = -yDistance;
                }
                // Constructs the relative point for the label
                return new Point_js_1.default(((totalLength / 2 - length - projlen) / totalLength) * -2, yDistance / this.scale);
            }
        }
        return new Point_js_1.default();
    }
    /**
     ...
     */
    createState(cell) {
        return new CellState_js_1.default(this, cell, this.graph.getCellStyle(cell));
    }
    /**
    ...
    isScrollEvent(evt) {
        ...
        const pt = new Point_js_1.default(eventClientPosition[0] - offset.x, eventClientPosition[1] - offset.y);
    ...
    installListeners() {
        ...
        if (Client_js_1.default.IS_TOUCH) {
            InternalEvent_js_1.default.addListener(container, 'gesturestart', ((evt) => {
                graph.fireGestureEvent(evt);
                InternalEvent_js_1.default.consume(evt);
            }));
            InternalEvent_js_1.default.addListener(container, 'gesturechange', ((evt) => {
                graph.fireGestureEvent(evt);
                InternalEvent_js_1.default.consume(evt);
            }));
            InternalEvent_js_1.default.addListener(container, 'gestureend', ((evt) => {
                graph.fireGestureEvent(evt);
                InternalEvent_js_1.default.consume(evt);
            }));
        }
        ...
        InternalEvent_js_1.default.addGestureListeners(container, ((evt) => {
            // Condition to avoid scrollbar events starting a rubberband selection
            if (this.isContainerEvent(evt) &&
                ((!Client_js_1.default.IS_GC && !Client_js_1.default.IS_SF) || !this.isScrollEvent(evt))) {
                graph.fireMouseEvent(InternalEvent_js_1.default.MOUSE_DOWN, new InternalMouseEvent_js_1.default(evt));
                // @ts-ignore
        ...
        InternalEvent_js_1.default.addListener(container, 'dblclick', ((evt) => {
            if (this.isContainerEvent(evt)) {
                graph.dblClick(evt);
            }
        }));
        ...
        const getState = (evt) => {
            ...
            if (Client_js_1.default.IS_TOUCH) {
                ...
                const pt = (0, styleUtils_js_1.convertPoint)(container, x, y);
        ...
        this.moveHandler = (evt) => {
            ...
            if (this.captureDocumentGesture &&
                ...
                !(0, EventUtils_js_1.isConsumed)(evt)) {
        ...
        InternalEvent_js_1.default.addGestureListeners(document, null, this.moveHandler, this.endHandler);
    }
    /**
    ...
    createSvg() {
        ...
        const canvas = (this.canvas = document.createElementNS(Constants_js_1.NS_SVG, 'g'));
        // For background image
        this.backgroundPane = document.createElementNS(Constants_js_1.NS_SVG, 'g');
        canvas.appendChild(this.backgroundPane);
        // Adds two layers (background is early feature)
        this.drawPane = document.createElementNS(Constants_js_1.NS_SVG, 'g');
        canvas.appendChild(this.drawPane);
        this.overlayPane = document.createElementNS(Constants_js_1.NS_SVG, 'g');
        canvas.appendChild(this.overlayPane);
        this.decoratorPane = document.createElementNS(Constants_js_1.NS_SVG, 'g');
        canvas.appendChild(this.decoratorPane);
        const root = document.createElementNS(Constants_js_1.NS_SVG, 'svg');
        root.style.left = '0px';
        root.style.top = '0px';
    ...
    updateContainerStyle(container) {
        ...
        const style = (0, styleUtils_js_1.getCurrentStyle)(container);
        if (style != null && style.position == 'static') {
            container.style.position = 'relative';
        }
        // Disables built-in pan and zoom in IE10 and later
        if (Client_js_1.default.IS_POINTER) {
    ...
    destroy() {
        ...
        if (root && root.parentNode) {
            ...
            InternalEvent_js_1.default.removeGestureListeners(document, null, this.moveHandler, this.endHandler);
            InternalEvent_js_1.default.release(this.graph.container);
