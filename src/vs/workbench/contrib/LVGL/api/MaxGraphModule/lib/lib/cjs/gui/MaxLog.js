...
const guiUtils_js_1 = require("./guiUtils.js");
const copyTextToClipboard = (text) => {
    navigator.clipboard.writeText(text).then(function () {
        config_js_1.GlobalConfig.logger.info('Async: Copying to clipboard was successful!');
    }, function (err) {
        config_js_1.GlobalConfig.logger.error('Async: Could not copy text: ', err);
    });
};
/**
...
class MaxLog {
    ...
    static init() {
        if (MaxLog.window == null && document.body != null) {
            const title = `${MaxLog.consoleName} - mxGraph ${Constants_js_1.VERSION}`;
            ...
            if (Client_js_1.default.IS_NS && document.compatMode !== 'BackCompat') {
            ...
            MaxLog.addButton('DOM', function (evt) {
                const content = (0, domUtils_js_1.getInnerHtml)(document.body);
            ...
            MaxLog.addButton('Show', function (evt) {
                try {
                    (0, guiUtils_js_1.popup)(MaxLog.textarea.value);
                }
                catch (err) {
                    alert(err);
                }
            ...
            MaxLog.window = new MaxWindow_js_1.default(title, table, Math.max(0, w - 320), Math.max(0, h - 210), 300, 160);
            ...
            if (Client_js_1.default.IS_NS &&
                !Client_js_1.default.IS_GC &&
                !Client_js_1.default.IS_SF &&
                ...
                MaxLog.window.addListener(InternalEvent_js_1.default.RESIZE_END, resizeHandler);
                MaxLog.window.addListener(InternalEvent_js_1.default.MAXIMIZE, resizeHandler);
                MaxLog.window.addListener(InternalEvent_js_1.default.NORMALIZE, resizeHandler);
     ...
     */
    static info() {
        MaxLog.writeln((0, StringUtils_js_1.toString)(navigator));
    }
    /**
    ...
    static addButton(lab, funct) {
        ...
        (0, domUtils_js_1.write)(button, lab);
        InternalEvent_js_1.default.addListener(button, 'click', funct);
    ...
    static leave(string, t0) {
        if (MaxLog.TRACE) {
            const dt = (0, time_utils_js_1.getElapseMillisecondsMessage)(t0);
