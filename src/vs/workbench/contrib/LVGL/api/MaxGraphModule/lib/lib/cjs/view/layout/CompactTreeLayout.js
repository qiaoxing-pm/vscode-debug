...
exports.CompactTreeLayout = void 0;
...
const treeTraversal_js_1 = require("../../util/treeTraversal.js");
/**
 ...
 * @category Layout
 */
class CompactTreeLayout extends GraphLayout_js_1.default {
    ...
    execute(parent, root) {
        ...
        if (root == null) {
            ...
            // children
                ...
                const roots = (0, treeTraversal_js_1.findTreeRoots)(this.graph, parent, true, this.invert);
                if (roots.length > 0) {
    ...
    sortOutgoingEdges(source, edges) {
        ...
        edges.sort((e1, e2) => {
            ...
            if (p1 == null) {
                p1 = CellPath_js_1.default.create(end1).split(CellPath_js_1.default.PATH_SEPARATOR);
                lookup.set(end1, p1);
            }
            const end2 = e2.getTerminal(e2.getTerminal(false) === source);
            let p2 = lookup.get(end2);
            if (p2 == null) {
                p2 = CellPath_js_1.default.create(end2).split(CellPath_js_1.default.PATH_SEPARATOR);
                lookup.set(end2, p2);
            }
            return CellPath_js_1.default.compare(p1, p2);
        });
    }
    /**
     ...
     */
    dfs(cell, parent) {
        const id = CellPath_js_1.default.create(cell);
        let node = null;
        ...
        if (cell != null && g != null) {
            if (this.isVertexMovable(cell)) {
                ...
                if (this.resizeParent) {
                    ...
                    const id = CellPath_js_1.default.create(parent);
            ...
            if (bounds == null) {
                bounds = new Rectangle_js_1.default(g.x, g.y, g.width, g.height);
            }
            else {
                bounds = new Rectangle_js_1.default(Math.min(bounds.x, g.x), Math.min(bounds.y, g.y), Math.max(bounds.x + bounds.width, g.x + g.width), Math.max(bounds.y + bounds.height, g.y + g.height));
            }
        }
        return bounds;
    }
    ...
    adjustParents() {
        ...
        this.arrangeGroups((0, styleUtils_js_1.sortCells)(tmp, true), this.groupPadding, this.groupPaddingTop, this.groupPaddingRight, this.groupPaddingBottom, this.groupPaddingLeft);
    }
    /**
     * Moves the specified node and all of its children by the given amount.
     */
    ...
    processNodeOutgoing(node) {
        ...
        while (child != null) {
            ...
            sortedCells.push(new WeightedCellSorter_js_1.default(child, sortingCriterion));
            child = child.next;
        }
...
exports.default = CompactTreeLayout;
