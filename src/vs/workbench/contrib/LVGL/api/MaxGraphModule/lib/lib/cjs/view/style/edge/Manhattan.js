...
exports.ManhattanConnector = void 0;
...
const Orthogonal_js_1 = require("./Orthogonal.js");
...
const ManhattanConnector = (state, source, target, points, result) => {
    ...
    function getRectangleCenter(rect) {
        return new Point_js_1.default(rect.x + rect.width / 2, rect.y + rect.height / 2);
    }
    function getDifferencePoint(p1, p2) {
        return new Point_js_1.default(p1.x - p2.x, p1.y - p2.y);
    }
    ...
    function normalizePoint(point) {
        return new Point_js_1.default(point.x === 0 ? 0 : Math.abs(point.x) / point.x, point.y === 0 ? 0 : Math.abs(point.y) / point.y);
    }
    ...
    function toPointFromString(pointString) {
        ...
        return new Point_js_1.default(parseInt(xy[0], 10), parseInt(xy[1], 10));
    }
    ...
    function getCellAbsoluteBounds(cellState) {
        ...
        const res = new Rectangle_js_1.default(round(cellBounds.x / scale - x), round(cellBounds.y / scale - y), round(cellBounds.width / scale), round(cellBounds.height / scale));
        return res;
    }
    const mStep = config_js_1.ManhattanConnectorConfig.step;
    const config = {
        // Padding applied on the element bounding boxes
        paddingBox: new Geometry_js_1.default(-mStep, -mStep, mStep * 2, mStep * 2),
        ...
        directions: [
            {
                ...
                offsetY: 0,
                cost: mStep,
                angle: normalizeAngle(getPointTheta(new Point_js_1.default(0, 0), new Point_js_1.default(mStep, 0))),
            },
            {
                offsetX: 0,
                offsetY: mStep,
                cost: mStep,
                angle: normalizeAngle(getPointTheta(new Point_js_1.default(0, 0), new Point_js_1.default(0, mStep))),
            },
            {
                offsetX: -mStep,
                offsetY: 0,
    ...
    class ObstacleMap {
        ...
        build(source, target) {
            ...
            return Array.from(graph.getView().getCellStates())
                ...
                if (!bbox)
                    return;
                const origin = snapPointToGrid(new Point_js_1.default(bbox.x, bbox.y), this.mapGridSize);
                const corner = snapPointToGrid(new Point_js_1.default(bbox.x + bbox.width, bbox.y + bbox.height), this.mapGridSize);
    ...
    function getRectPoints(bbox, directions, opt) {
        const step = config_js_1.ManhattanConnectorConfig.step;
    ...
    function alignPointToCell(point, edgeState, cellState, isSourceCell) {
        ...
        const onlyHorizontalDirections = isSourceCell
            ? config_js_1.ManhattanConnectorConfig.startDirections.every((d) => d != 'north' && d != 'south')
            : config_js_1.ManhattanConnectorConfig.endDirections.every((d) => d != 'north' && d != 'south');
        ...
        const onlyVerticalDirections = isSourceCell
            ? config_js_1.ManhattanConnectorConfig.startDirections.every((d) => d != 'west' && d != 'east')
            : config_js_1.ManhattanConnectorConfig.endDirections.every((d) => d != 'west' && d != 'east');
    ...
    function findRoute(start, end, obstacleMap, opt) {
        ...
        const step = config_js_1.ManhattanConnectorConfig.step;
        const startPoints = getRectPoints(start, config_js_1.ManhattanConnectorConfig.startDirections, opt).filter((p) => obstacleMap.isPointAccessible(p));
        const startCenter = snapPointToGrid(getRectangleCenter(start), step);
        const endPoints = getRectPoints(end, config_js_1.ManhattanConnectorConfig.endDirections, opt).filter((p) => obstacleMap.isPointAccessible(p));
        ...
        if (startPoints.length > 0 && endPoints.length > 0) {
            ...
            let loopsRemain = config_js_1.ManhattanConnectorConfig.maxLoops;
            ...
            while (!openSet.isEmpty() && loopsRemain > 0) {
                ...
                for (let i = 0; i < opt.directions.length; i++) {
                    ...
                    if (previousDirectionAngle &&
                        directionChangedAngle > config_js_1.ManhattanConnectorConfig.maxAllowedDirectionChange) {
                        continue;
                    }
    ...
    function router(state, source, target, points, result, opt) {
        ...
        if ((points != null && points.length > 0) || source == null || target == null) {
            (0, Segment_js_1.SegmentConnector)(state, source, target, points, result);
            return;
        }
        ...
        if (!sourceBBox || !targetBBox) {
            ...
            return (0, Orthogonal_js_1.OrthogonalConnector)(state, source, target, points, result);
        }
        ...
        routePoints.forEach((pt) => result.push(new Point_js_1.default(Math.round((pt.x + state.view.translate.x) * scale * 10) / 10, Math.round((pt.y + state.view.translate.y) * scale * 10) / 10)));
    }
    router(state, source, target, points, result, config);
};
exports.ManhattanConnector = ManhattanConnector;
