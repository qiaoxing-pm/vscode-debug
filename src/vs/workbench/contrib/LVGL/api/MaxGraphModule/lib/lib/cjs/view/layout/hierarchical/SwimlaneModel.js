...
const GraphHierarchyNode_js_1 = __importDefault(require("../datatypes/GraphHierarchyNode.js"));
const GraphHierarchyEdge_js_1 = __importDefault(require("../datatypes/GraphHierarchyEdge.js"));
...
class SwimlaneModel {
    ...
    createInternalCells(layout, vertices, internalVertices) {
        ...
        for (let i = 0; i < vertices.length; i += 1) {
            internalVertices[i] = new GraphHierarchyNode_js_1.default(vertices[i]);
            ...
            for (let j = 0; j < conns.length; j += 1) {
                ...
                if (cell !== vertices[i] && cell.isVertex() && !layout.isVertexIgnored(cell)) {
                    ...
                    if (undirectedEdges != null &&
                        ...
                        const internalEdge = new GraphHierarchyEdge_js_1.default(undirectedEdges);
    ...
    maxChainDfs(parent, root, connectingEdge, seen, chainCount) {
        if (root != null) {
            const rootId = CellPath_js_1.default.create(root.cell);
            if (seen[rootId] == null) {
                seen[rootId] = root;
                ...
                for (let i = 0; i < outgoingEdges.length; i += 1) {
                    ...
                    if (root.swimlaneIndex < targetNode.swimlaneIndex) {
                        this.maxChainDfs(root, targetNode, internalEdge, (0, cloneUtils_js_1.clone)(seen, null, true), 0);
                    }
                    else if (root.swimlaneIndex === targetNode.swimlaneIndex) {
                        this.maxChainDfs(root, targetNode, internalEdge, (0, cloneUtils_js_1.clone)(seen, null, true), chainCount + 1);
                    }
                }
            }
        }
