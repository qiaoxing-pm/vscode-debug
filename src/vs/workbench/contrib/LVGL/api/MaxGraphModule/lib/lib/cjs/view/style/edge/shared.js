...
exports.scalePointArray = scalePointArray;
exports.scaleCellState = scaleCellState;
...
function scalePointArray(points, scale) {
    ...
    if (points != null) {
        for (let i = 0; i < points.length; i += 1) {
            if (points[i] != null) {
                result[i] = new Point_js_1.default(Math.round((points[i].x / scale) * 10) / 10, Math.round((points[i].y / scale) * 10) / 10);
            }
            else {
                result[i] = null;
            }
