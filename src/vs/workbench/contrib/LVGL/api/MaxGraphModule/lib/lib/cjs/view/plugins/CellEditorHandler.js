...
const TextShape_js_1 = __importDefault(require("../shape/node/TextShape.js"));
...
class CellEditorHandler {
    constructor(graph) {
         ...
         */
        this.emptyLabelText = Client_js_1.default.IS_FF ? '<br>' : '';
        /**
         ...
         */
        this.minResize = new Rectangle_js_1.default(0, 20);
        /**
        ...
        this.graph.getView().addListener(InternalEvent_js_1.default.SCALE, this.zoomHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, this.zoomHandler);
    ...
    getInitialValue(state, trigger) {
        let result = (0, StringUtils_js_1.htmlEntities)(this.graph.getEditingValue(state.cell, trigger), false);
        result = (0, StringUtils_js_1.replaceTrailingNewlines)(result, '<div><br></div>');
    ...
    getCurrentValue(state) {
        if (!this.textarea)
            return null;
        return (0, domUtils_js_1.extractTextWithWhitespace)(Array.from(this.textarea.childNodes));
    }
    /**
    ...
    isCancelEditingKeyEvent(evt) {
        return (this.escapeCancelsEditing ||
            (0, EventUtils_js_1.isShiftDown)(evt) ||
            (0, EventUtils_js_1.isControlDown)(evt) ||
            (0, EventUtils_js_1.isMetaDown)(evt));
    }
    /**
    ...
    installListeners(elt) {
        ...
        InternalEvent_js_1.default.addListener(elt, 'dragstart', (evt) => {
            this.graph.stopEditing(false);
            InternalEvent_js_1.default.consume(evt);
        });
        // Applies value if focus is lost
        InternalEvent_js_1.default.addListener(elt, 'blur', (evt) => {
            if (this.blurEnabled) {
                this.focusLost();
            }
        });
        // Updates modified state and handles placeholder text
        InternalEvent_js_1.default.addListener(elt, 'keydown', (evt) => {
        ...
        InternalEvent_js_1.default.addListener(elt, 'keypress', keypressHandler);
        InternalEvent_js_1.default.addListener(elt, 'paste', keypressHandler);
        ...
        InternalEvent_js_1.default.addListener(elt, 'input', keyupHandler);
        InternalEvent_js_1.default.addListener(elt, 'cut', keyupHandler);
        InternalEvent_js_1.default.addListener(elt, 'paste', keyupHandler);
        // Adds automatic resizing of the textbox while typing using input, keyup and/or DOM change events
        const evtName = 'input';
        const resizeHandler = (evt) => {
            if (this.editingCell != null && this.autoSize && !(0, EventUtils_js_1.isConsumed)(evt)) {
        ...
        InternalEvent_js_1.default.addListener(elt, evtName, resizeHandler);
        InternalEvent_js_1.default.addListener(window, 'resize', resizeHandler);
        InternalEvent_js_1.default.addListener(elt, 'cut', resizeHandler);
        InternalEvent_js_1.default.addListener(elt, 'paste', resizeHandler);
    }
    /**
     ...
     */
    isEventSource(evt) {
        return (0, EventUtils_js_1.getSource)(evt) === this.textarea;
    }
    /**
        ...
        if (!state) {
            ...
            if (!this.autoSize || state.style.overflow === 'fill') {
                ...
                if (this.graph.isWrapping(state.cell) &&
                    ...
                    this.textarea.style.wordWrap = Constants_js_1.WORD_WRAP;
                ...
                if (m == null) {
                    m = (0, styleUtils_js_1.getAlignmentAsPoint)(this.align ?? state.style.align ?? 'center', state.style.verticalAlign ?? 'middle');
                }
                if (isEdge) {
                    this.bounds = new Rectangle_js_1.default(state.absoluteOffset.x, state.absoluteOffset.y, 0, 0);
                    if (lw != null) {
                    ...
                    let bounds = Rectangle_js_1.default.fromRectangle(state);
                    ...
                    if (!state.view.graph.cellRenderer.legacySpacing ||
                        ...
                        // @ts-ignore
                        const dummy = new TextShape_js_1.default(); // FIXME!!!! ===================================================================================================
                        ...
                        vpos =
                            state.style.verticalLabelPosition != null
                                ...
                                : 'middle';
                        bounds = new Rectangle_js_1.default(bounds.x + spacingLeft, bounds.y + spacingTop, bounds.width -
                            (hpos === 'center' && lw == null ? spacingLeft + spacingRight : 0), bounds.height - (vpos === 'middle' ? spacingTop + spacingBottom : 0));
                    }
                    this.bounds = new Rectangle_js_1.default(bounds.x + state.absoluteOffset.x, bounds.y + state.absoluteOffset.y, bounds.width, bounds.height);
                }
            ...
            (0, styleUtils_js_1.setPrefixedStyle)(this.textarea.style, 'transformOrigin', '0px 0px');
            (0, styleUtils_js_1.setPrefixedStyle)(this.textarea.style, 'transform', `scale(${scale},${scale})${m == null ? '' : ` translate(${m.x * 100}%,${m.y * 100}%)`}`);
        }
    }
    /**
    ...
    startEditing(cell, trigger = null) {
        ...
        if (state) {
            ...
            const size = stateStyle.fontSize ?? Constants_js_1.DEFAULT_FONTSIZE;
            const family = stateStyle.fontFamily ?? Constants_js_1.DEFAULT_FONTFAMILY;
            const color = stateStyle.fontColor ?? 'black';
            const align = stateStyle.align ?? 'left';
            const fontStyle = stateStyle.fontStyle ?? 0;
            const bold = (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.BOLD);
            const italic = (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.ITALIC);
            const txtDecor = [];
            (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.UNDERLINE) && txtDecor.push('underline');
            (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.STRIKETHROUGH) &&
                txtDecor.push('line-through');
            const textarea = this.textarea; // code above ensure it is always set
            const textareaStyle = textarea.style;
            textareaStyle.lineHeight = Constants_js_1.ABSOLUTE_LINE_HEIGHT
                ? `${Math.round(size * Constants_js_1.LINE_HEIGHT)}px`
                : String(Constants_js_1.LINE_HEIGHT);
            ...
            let dir = (this.textDirection = stateStyle.textDirection ?? Constants_js_1.DEFAULT_TEXT_DIRECTION);
            const stateText = state.text;
            if (dir === 'auto') {
                if (stateText !== null &&
                    stateText.dialect !== 'strictHtml' &&
                    !(0, domUtils_js_1.isNode)(stateText.value)) {
    ...
    stopEditing(cancel = false) {
        if (this.editingCell) {
            ...
            this.bounds = null;
            textarea.blur();
            (0, domUtils_js_1.clearSelection)();
            ...
            if (this.textarea)
                InternalEvent_js_1.default.release(this.textarea);
            this.textarea = null;
            this.align = null;
        }
    }
    ...
    getMinimumSize(state) {
        const { scale } = this.graph.getView();
        const textarea = this.textarea;
        return new Rectangle_js_1.default(0, 0, state.text === null ? 30 : state.text.size * scale + 20, textarea.style.textAlign === 'left' ? 120 : 40);
    }
    /**
    ...
    getEditorBounds(state) {
        ...
        if (!isEdge &&
            ...
            result = state.shape.getLabelBounds(Rectangle_js_1.default.fromRectangle(state));
        }
        else {
            // @ts-ignore
            const dummy = new TextShape_js_1.default(); // FIXME!!!! ===================================================================================================
            ...
            const spacingLeft = ((state.style.spacingLeft ?? 0) + dummy.baseSpacingLeft) * scale + spacing;
            result = new Rectangle_js_1.default(state.x, state.y, Math.max(minWidth, state.width - spacingLeft - spacingRight), Math.max(minHeight, state.height - spacingTop - spacingBottom));
        ...
        return new Rectangle_js_1.default(Math.round(result.x), Math.round(result.y), Math.round(result.width), Math.round(result.height));
    }
    /**
...
exports.default = CellEditorHandler;
