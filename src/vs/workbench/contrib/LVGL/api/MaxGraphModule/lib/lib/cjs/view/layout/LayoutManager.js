 ...
 * @category Layout
 */
class LayoutManager extends EventSource_js_1.default {
    constructor(graph) {
        super();
        /**
    ...
    setGraph(graph) {
        ...
        if (this.graph) {
            const model = this.graph.getDataModel();
            model.addListener(InternalEvent_js_1.default.BEFORE_UNDO, this.undoHandler);
            this.graph.addListener(InternalEvent_js_1.default.MOVE_CELLS, this.moveHandler);
            this.graph.addListener(InternalEvent_js_1.default.RESIZE_CELLS, this.resizeHandler);
        }
    }
    /**
     ...
     */
    hasLayout(cell) {
        return !!this.getLayout(cell, InternalEvent_js_1.default.LAYOUT_CELLS);
    }
    /**
    ...
    cellsMoved(cells, evt) {
        if (cells.length > 0 && evt) {
            const point = (0, styleUtils_js_1.convertPoint)(this.getGraph().container, (0, EventUtils_js_1.getClientX)(evt), (0, EventUtils_js_1.getClientY)(evt));
            for (let i = 0; i < cells.length; i += 1) {
                const layout = this.getLayout(cells[i].getParent(), InternalEvent_js_1.default.MOVE_CELLS);
                if (layout) {
    ...
    cellsResized(cells = null, bounds = null, prev = null) {
        if (cells && bounds) {
            for (let i = 0; i < cells.length; i += 1) {
                const layout = this.getLayout(cells[i].getParent(), InternalEvent_js_1.default.RESIZE_CELLS);
                if (layout) {
    ...
    getCellsForChange(change) {
        if (change instanceof ChildChange_js_1.default) {
            return this.addCellsWithLayout(change.child, this.addCellsWithLayout(change.previous));
        }
        if (change instanceof TerminalChange_js_1.default || change instanceof GeometryChange_js_1.default) {
            return this.addCellsWithLayout(change.cell);
        }
        if (change instanceof VisibleChange_js_1.default || change instanceof StyleChange_js_1.default) {
            return this.addCellsWithLayout(change.cell);
        }
        return [];
    }
    ...
    executeLayoutForCells(cells) {
        const sorted = (0, styleUtils_js_1.sortCells)(cells, false);
    ...
    layoutCells(cells, bubble = false) {
        if (cells.length > 0) {
            ...
            model.batchUpdate(() => {
                ...
                this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.LAYOUT_CELLS, { cells }));
            });
        }
    }
    /**
    ...
    executeLayout(cell, bubble = false) {
        const layout = this.getLayout(cell, bubble ? InternalEvent_js_1.default.BEGIN_UPDATE : InternalEvent_js_1.default.END_UPDATE);
        if (layout) {
            layout.execute(cell);
        }
    }
