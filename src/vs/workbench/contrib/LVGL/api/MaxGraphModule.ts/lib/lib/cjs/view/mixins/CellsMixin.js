            ...
            if (parent) {
                for (const cell of parent.getChildren()) {
                    ...
                    if (state && cell.isVisible() && (!ignoreFn || !ignoreFn(state))) {
                        ...
                        const hit = (intersection && cell.isVertex() && (0, mathUtils_js_1.intersects)(intersection, box)) ||
                            (!intersection &&
    ...
    intersects(state, x, y) {
        ...
        if (pts.length > 0) {
            ...
            for (let i = 1; i < pts.length; i += 1) {
                const next = pts[i];
                if (pt && next) {
                    const dist = (0, mathUtils_js_1.ptSegDistSq)(pt.x, pt.y, next.x, next.y, x, y);
                    if (dist <= t2) {
                        return true;
                    }
                }
            ...
            if (alpha !== 0) {
                ...
                const pt = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(x, y), cos, sin, cx);
                x = pt.x;
                y = pt.y;
            }
            if ((0, mathUtils_js_1.contains)(state, x, y)) {
                return true;
            }
        }
        return false;
                ...
                if (geo) {
                    ...
                    if (cell.isEdge()) {
                        const addPoint = (pt) => {
                            if (pt) {
                                if (!tmp) {
                                    tmp = new Rectangle_js_1.default(pt.x, pt.y, 0, 0);
                                }
                                else {
                                    tmp.add(new Rectangle_js_1.default(pt.x, pt.y, 0, 0));
                                }
                            }
                        };
                        ...
                        if (pts && pts.length > 0) {
                            tmp = new Rectangle_js_1.default(pts[0].x, pts[0].y, 0, 0);
                        ...
                        if (geo.relative && parent) {
                            if (parent.isVertex() && parent !== this.getView().currentRoot) {
                                ...
                                if (tmp) {
                                    bbox = new Rectangle_js_1.default(geo.x * tmp.width, geo.y * tmp.height, geo.width, geo.height);
                            ...
                            bbox = Rectangle_js_1.default.fromRectangle(geo);
                        ...
                        if (bbox) {
                            ...
                            if (angle !== 0) {
                                bbox = (0, mathUtils_js_1.getBoundingBox)(bbox, angle);
                            }
                        }
                    }
                    if (bbox) {
