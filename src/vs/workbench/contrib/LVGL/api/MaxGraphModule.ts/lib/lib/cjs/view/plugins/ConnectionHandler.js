...
const ConstraintHandler_js_1 = __importDefault(require("../handler/ConstraintHandler.js"));
 ...
 * @category Plugin
 */
class ConnectionHandler extends EventSource_js_1.default {
    /**
    ...
    constructor(graph, factoryMethod = null) {
         ...
         */
        this.connectIconOffset = new Point_js_1.default(0, Constants_js_1.TOOLTIP_VERTICAL_OFFSET);
        /**
        ...
        this.graph.getDataModel().addListener(InternalEvent_js_1.default.CHANGE, this.changeHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.SCALE, this.changeHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.TRANSLATE, this.changeHandler);
        this.graph
            .getView()
            .addListener(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, this.changeHandler);
        // Removes the icon if we step into/up or start editing
        this.drillHandler = (sender) => {
            this.reset();
        };
        this.graph.addListener(InternalEvent_js_1.default.START_EDITING, this.drillHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.DOWN, this.drillHandler);
        this.graph.getView().addListener(InternalEvent_js_1.default.UP, this.drillHandler);
        // Handles escape keystrokes
        this.escapeHandler = () => {
            this.reset();
        };
        this.graph.addListener(InternalEvent_js_1.default.ESCAPE, this.escapeHandler);
    }
    /**
    ...
    createShape() {
        ...
        const shape = this.livePreview && this.edgeState
            ...
            : new PolylineShape_js_1.default([], Constants_js_1.INVALID_COLOR);
        if (shape && shape.node) {
            shape.dialect = 'svg';
            ...
            InternalEvent_js_1.default.redirectMouseEvents(shape.node, this.graph, null);
        }
        return shape;
    }
    /**
    ...
    start(state, x, y, edgeState) {
        this.previous = state;
        this.first = new Point_js_1.default(x, y);
        this.edgeState = edgeState ?? this.createEdgeState();
        // Marks the source state
        this.marker.currentColor = this.marker.validColor;
        this.marker.markedState = state;
        this.marker.mark();
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.START, { state: this.previous }));
    }
    /**
    ...
    createIcons(state) {
        ...
        if (image) {
            ...
            const bounds = new Rectangle_js_1.default(0, 0, image.width, image.height);
            const icon = new ImageShape_js_1.default(bounds, image.src, undefined, undefined, 0);
            ...
            const mouseDown = (evt) => {
                if (!(0, EventUtils_js_1.isConsumed)(evt)) {
                    this.icon = icon;
                    this.graph.fireMouseEvent(InternalEvent_js_1.default.MOUSE_DOWN, new InternalMouseEvent_js_1.default(evt, getState()));
                }
            };
            InternalEvent_js_1.default.redirectMouseEvents(icon.node, this.graph, getState, mouseDown);
            icons.push(icon);
    ...
    getIconPosition(icon, state) {
        ...
        if (this.graph.isSwimlane(state.cell)) {
            ...
            const alpha = (0, mathUtils_js_1.toRadians)(state.style.rotation ?? 0);
            if (alpha !== 0) {
                const cos = Math.cos(alpha);
                const sin = Math.sin(alpha);
                const ct = new Point_js_1.default(state.getCenterX(), state.getCenterY());
                const pt = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(cx, cy), cos, sin, ct);
                cx = pt.x;
                cy = pt.y;
            }
        }
        return new Point_js_1.default(cx - icon.bounds.width / 2, cy - icon.bounds.height / 2);
    }
    /**
    ...
    mouseDown(_sender, me) {
        ...
        if (this.isEnabled() &&
            ...
            if (this.constraintHandler.currentConstraint &&
                ...
                this.first = new Point_js_1.default(me.getGraphX(), me.getGraphY());
            }
            ...
            if (this.outlineConnect) {
                if (this.marker.highlight != null && this.marker.highlight.shape != null) {
                    ...
                    if (this.constraintHandler.currentConstraint != null &&
                        ...
                        this.marker.highlight.shape.stroke = Constants_js_1.OUTLINE_HIGHLIGHT_COLOR;
                        this.marker.highlight.shape.strokeWidth =
                            Constants_js_1.OUTLINE_HIGHLIGHT_STROKEWIDTH / s / s;
                        this.marker.highlight.repaint();
                    }
                        ...
                        if (cell &&
                            ...
                            this.marker.highlight.shape.stroke = Constants_js_1.DEFAULT_VALID_COLOR;
                        }
    ...
    snapToPreview(me, point) {
        if (!(0, EventUtils_js_1.isAltDown)(me.getEvent()) && this.previous) {
            const tol = (this.graph.getGridSize() * this.graph.view.scale) / 2;
            const tmp = this.sourceConstraint && this.first
                ? this.first
                : new Point_js_1.default(this.previous.getCenterX(), this.previous.getCenterY());
    ...
    mouseMove(_sender, me) {
        if (!me.isConsumed() &&
            ...
            let point = new Point_js_1.default(me.getGraphX(), me.getGraphY());
            this.error = null;
            if (this.graph.isGridEnabledEvent(me.getEvent())) {
                point = new Point_js_1.default((this.graph.snap(point.x / scale - tr.x) + tr.x) * scale, (this.graph.snap(point.y / scale - tr.y) + tr.y) * scale);
            }
            ...
            if (this.first) {
                ...
                if (this.selectedIcon && this.selectedIcon.bounds) {
                    ...
                    if (this.currentState && this.targetConnectImage) {
                        ...
                        const bounds = new Rectangle_js_1.default(me.getGraphX() + this.connectIconOffset.x, me.getGraphY() + this.connectIconOffset.y, w, h);
    ...
    getTargetPerimeterPoint(state, _me) {
        ...
        if (targetPerimeter && this.previous && this.edgeState) {
            ...
            result = new Point_js_1.default(state.getCenterX(), state.getCenterY());
        }
        return result;
    }
    /**
    ...
    getSourcePerimeterPoint(state, next, me) {
        ...
        const c = new Point_js_1.default(state.getCenterX(), state.getCenterY());
        if (sourcePerimeter) {
            const theta = state.style.rotation ?? 0;
            const rad = -theta * (Math.PI / 180);
            if (theta !== 0) {
                next = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(next.x, next.y), Math.cos(rad), Math.sin(rad), c);
            }
            let tmp = sourcePerimeter(view.getPerimeterBounds(state), state, next, false);
            if (tmp) {
                if (theta !== 0) {
                    tmp = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(tmp.x, tmp.y), Math.cos(-rad), Math.sin(-rad), c);
                }
                result = tmp;
            }
        }
    ...
    addWaypointForEvent(me) {
        if (!this.first)
            return;
        let point = (0, styleUtils_js_1.convertPoint)(this.graph.container, me.getX(), me.getY());
        ...
        if (addPoint) {
            ...
            point = new Point_js_1.default(this.graph.snap(me.getGraphX() / scale) * scale, this.graph.snap(me.getGraphY() / scale) * scale);
    ...
    reset() {
        ...
        this.mouseDownCounter = 0;
        this.first = null;
        this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.RESET));
    }
    /**
     ...
     */
    getEdgeColor(valid) {
        return valid ? Constants_js_1.VALID_COLOR : Constants_js_1.INVALID_COLOR;
    }
    /**
                ...
                if (edge && source) {
                    ...
                    if (this.waypoints.length > 0) {
                        ...
                        for (let i = 0; i < this.waypoints.length; i += 1) {
                            ...
                            geo.points.push(new Point_js_1.default(pt.x / s - tr.x, pt.y / s - tr.y));
                        }
                    }
                    if (!target && this.currentPoint) {
                        ...
                        const pt = this.originalPoint != null
                            ? new Point_js_1.default(this.originalPoint.x / s - t.x, this.originalPoint.y / s - t.y)
                            : new Point_js_1.default(this.currentPoint.x / s - t.x, this.currentPoint.y / s - t.y);
                        pt.x -= this.graph.getPanDx() / this.graph.view.scale;
                        pt.y -= this.graph.getPanDy() / this.graph.view.scale;
                        geo.setTerminalPoint(pt, false);
                    }
                    this.fireEvent(new EventObject_js_1.default(InternalEvent_js_1.default.CONNECT, 'cell', edge, 'terminal', target, 'event', evt, 'target', dropTarget, 'terminalInserted', terminalInserted));
                }
            }
            catch (e) {
                config_js_1.GlobalConfig.logger.show();
                const errorMessage = `Error in ConnectionHandler: ${e instanceof Error ? e.message + '\n' + e.stack : 'unknown cause'}`;
                config_js_1.GlobalConfig.logger.debug(errorMessage);
            }
            finally {
                model.endUpdate();
            }
    ...
    createTargetVertex(evt, source) {
        ...
        if (geo && this.currentPoint) {
            const t = this.graph.view.translate;
            const s = this.graph.view.scale;
            const point = new Point_js_1.default(this.currentPoint.x / s - t.x, this.currentPoint.y / s - t.y);
    ...
    createEdge(value, source, target, style = {}) {
        ...
        if (edge == null) {
            edge = new Cell_js_1.default(value || '');
            edge.setEdge(true);
            edge.setStyle(style);
...
class ConnectionHandlerCellMarker extends CellMarker_js_1.default {
    constructor(graph, connectionHandler, validColor = Constants_js_1.DEFAULT_VALID_COLOR, invalidColor = Constants_js_1.DEFAULT_INVALID_COLOR, hotspot = Constants_js_1.DEFAULT_HOTSPOT) {
