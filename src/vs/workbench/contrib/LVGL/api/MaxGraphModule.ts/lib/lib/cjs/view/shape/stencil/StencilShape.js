...
exports.StencilShapeConfig = void 0;
...
const StencilShapeRegistry_js_1 = require("./StencilShapeRegistry.js");
 ...
 * @category Shape
 */
class StencilShape extends Shape_js_1.default {
    constructor(desc) {
        super();
        /**
    ...
    parseDescription() {
        ...
        this.strokeWidthValue = !(0, utils_js_1.isNullish)(sw) ? sw : '1';
    }
    /**
    ...
    parseConstraints() {
        ...
        if (conns) {
            const tmp = (0, domUtils_js_1.getChildNodes)(conns);
            if (tmp.length > 0) {
                this.constraints = [];
    ...
    parseConstraint(node) {
        ...
        return new ConnectionConstraint_js_1.default(new Point_js_1.default(x, y), perimeter, name);
    }
    /**
    ...
    evaluateTextAttribute(node, attribute, shape) {
        ...
        if ((exports.StencilShapeConfig.defaultLocalized && !loc) || loc === '1') {
            result = (0, i18n_utils_js_1.translate)(result);
        }
        return result;
    }
    /**
    ...
    evaluateAttribute(node, attribute, shape) {
        ...
        if (!result) {
            const text = (0, domUtils_js_1.getTextContent)(node);
            if (text && exports.StencilShapeConfig.allowEval) {
                const funct = (0, utils_js_1.doEval)(text);
    ...
    drawShape(canvas, shape, x, y, w, h) {
        ...
        if (shape.style?.pointerEvents ?? false) {
            canvas.setStrokeColor(Constants_js_1.NONE);
            canvas.rect(x, y, w, h);
            canvas.stroke();
    ...
    computeAspect(shape = null, x, y, w, h, direction) {
        ...
        return new Rectangle_js_1.default(x0, y0, sx, sy);
    }
    /**
     * Draws this stencil inside the given bounds.
     */
            ...
            if (name === 'path') {
                ...
                if (node.getAttribute('rounded') == '1') {
                    ...
                    while (childNode != null) {
                        if ((0, utils_js_1.isElement)(childNode)) {
                            const childName = childNode.nodeName;
                            if (childName === 'move' || childName === 'line') {
                                if (childName === 'move' || segs.length === 0) {
                                    segs.push([]);
                                }
                                segs[segs.length - 1].push(new Point_js_1.default(x0 + Number(childNode.getAttribute('x')) * sx, y0 + Number(childNode.getAttribute('y')) * sy));
                                pointCount++;
                            }
                            else {
                ...
                if (arcsize === 0) {
                    arcsize = Constants_js_1.RECTANGLE_ROUNDING_FACTOR * 100;
                }
                ...
                const stencil = StencilShapeRegistry_js_1.StencilShapeRegistry.get(node.getAttribute('name'));
                if (stencil) {
