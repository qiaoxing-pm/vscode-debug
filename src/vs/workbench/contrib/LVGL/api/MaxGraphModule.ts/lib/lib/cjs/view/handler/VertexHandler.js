...
class VertexHandler {
     ...
     */
    isRotationEnabled() {
        return config_js_1.VertexHandlerConfig.rotationEnabled;
    }
    /**
    ...
    constructor(state) {
        ...
        this.edgeHandlers = [];
        this.EMPTY_POINT = new Point_js_1.default();
        this.state = state;
        this.graph = this.state.view.graph;
        this.selectionBounds = this.getSelectionBounds(this.state);
        this.bounds = new Rectangle_js_1.default(this.selectionBounds.x, this.selectionBounds.y, this.selectionBounds.width, this.selectionBounds.height);
        ...
        InternalEvent_js_1.default.redirectMouseEvents(this.selectionBorder.node, this.graph, this.state);
        if (this.graph.isCellMovable(this.state.cell)) {
            this.selectionBorder.setCursor(config_js_1.VertexHandlerConfig.cursorMovable);
        }
        ...
        if (selectionHandler &&
            ...
            if (resizable ||
                ...
                if (geo != null &&
                    ...
                    this.labelShape = this.createSizer(config_js_1.HandleConfig.labelCursor, InternalEvent_js_1.default.LABEL_HANDLE, config_js_1.HandleConfig.labelSize, config_js_1.HandleConfig.labelFillColor);
                ...
                this.state.width < 2 &&
                this.state.height < 2) {
                this.labelShape = this.createSizer(config_js_1.VertexHandlerConfig.cursorMovable, InternalEvent_js_1.default.LABEL_HANDLE, undefined, config_js_1.HandleConfig.labelFillColor);
                this.sizers.push(this.labelShape);
            }
        }
        // Adds the rotation handler
        if (this.isRotationHandleVisible()) {
            this.rotationShape = this.createSizer(this.rotationCursor, InternalEvent_js_1.default.ROTATION_HANDLE, config_js_1.HandleConfig.size + 3, config_js_1.HandleConfig.fillColor);
    ...
    isConstrainedEvent(me) {
        return (0, EventUtils_js_1.isShiftDown)(me.getEvent()) || this.state.style.aspect === 'fixed';
    }
    /**
    ...
    getSelectionBounds(state) {
        return new Rectangle_js_1.default(Math.round(state.x), Math.round(state.y), Math.round(state.width), Math.round(state.height));
    }
    /**
     ...
     */
    getSelectionColor() {
        return config_js_1.VertexHandlerConfig.selectionColor;
    }
    /**
     * Returns {@link VertexHandlerConfig.selectionStrokeWidth}.
     */
    getSelectionStrokeWidth() {
        return config_js_1.VertexHandlerConfig.selectionStrokeWidth;
    }
    /**
     * Returns {@link VertexHandlerConfig.selectionDashed}.
     */
    isSelectionDashed() {
        return config_js_1.VertexHandlerConfig.selectionDashed;
    }
    /**
     * Creates a sizer handle for the specified cursor and index and returns
     * the new {@link RectangleShape} that represents the handle.
     */
    createSizer(cursor, index, size = config_js_1.HandleConfig.size, fillColor = config_js_1.HandleConfig.fillColor) {
        const bounds = new Rectangle_js_1.default(0, 0, size, size);
        ...
        InternalEvent_js_1.default.redirectMouseEvents(sizer.node, this.graph, this.state);
     ...
     */
    createSizerShape(bounds, index, fillColor = config_js_1.HandleConfig.fillColor) {
        if (this.handleImage) {
            bounds = new Rectangle_js_1.default(bounds.x, bounds.y, this.handleImage.width, this.handleImage.height);
            const shape = new ImageShape_js_1.default(bounds, this.handleImage.src);
            // Allows HTML rendering of the images
            shape.preserveImageAspect = false;
            return shape;
        }
        const strokeColor = config_js_1.HandleConfig.strokeColor;
        if (index === InternalEvent_js_1.default.ROTATION_HANDLE) {
            return new EllipseShape_js_1.default(bounds, fillColor, strokeColor);
        }
        return new RectangleShape_js_1.default(bounds, fillColor, strokeColor);
    }
    /**
    ...
    getHandleForEvent(me) {
        ...
        const checkShape = (shape) => {
            const st = shape && shape.constructor !== ImageShape_js_1.default && this.allowHandleBoundsCheck
                ? shape.strokeWidth + shape.svgStrokeTolerance
                : null;
            const real = st
                ? new Rectangle_js_1.default(me.getGraphX() - Math.floor(st / 2), me.getGraphY() - Math.floor(st / 2), st, st)
                : hit;
            return (shape &&
                shape.bounds &&
                (me.isSource(shape) ||
                    (real &&
                        (0, mathUtils_js_1.intersects)(shape.bounds, real) &&
                        shape.node.style.display !== 'none' &&
                        shape.node.style.visibility !== 'hidden')));
        };
        if (checkShape(this.rotationShape)) {
            return InternalEvent_js_1.default.ROTATION_HANDLE;
        }
        if (checkShape(this.labelShape)) {
            return InternalEvent_js_1.default.LABEL_HANDLE;
        }
    ...
    isLivePreviewBorder() {
        return (this.state.shape &&
            this.state.shape.fill === Constants_js_1.NONE &&
            this.state.shape.stroke === Constants_js_1.NONE);
    }
    /**
    ...
    start(x, y, index) {
        ...
        this.startX = x;
        this.startY = y;
        if (this.index <= InternalEvent_js_1.default.CUSTOM_HANDLE && this.isGhostPreview()) {
            ...
            this.selectionBorder.node.style.display =
                index === InternalEvent_js_1.default.ROTATION_HANDLE ? 'inline' : 'none';
            // Creates the border that represents the new bounds
            if (!this.livePreviewActive || this.isLivePreviewBorder()) {
                this.preview = this.createSelectionShape(this.bounds);
                if (!(Client_js_1.default.IS_SVG && (this.state.style.rotation ?? 0) != 0) &&
            ...
            if (this.livePreviewActive) {
                this.hideSizers();
                if (index === InternalEvent_js_1.default.ROTATION_HANDLE && this.rotationShape) {
                    this.rotationShape.node.style.display = '';
                }
                else if (index === InternalEvent_js_1.default.LABEL_HANDLE && this.labelShape) {
                    ...
                    this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - index].setVisible(true);
                }
    ...
    checkTolerance(me) {
        if (this.inTolerance && this.startX !== null && this.startY !== null) {
            if ((0, EventUtils_js_1.isMouseEvent)(me.getEvent()) ||
            ...
            if (!this.inTolerance) {
                if (this.index <= InternalEvent_js_1.default.CUSTOM_HANDLE) {
                    if (this.customHandles != null) {
                        ...
                        this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - this.index].active = true;
                ...
                else if (this.index === InternalEvent_js_1.default.LABEL_HANDLE) {
                    this.moveLabel(me);
                }
                else {
                    if (this.index === InternalEvent_js_1.default.ROTATION_HANDLE) {
     ...
     */
    moveLabel(me) {
        const point = new Point_js_1.default(me.getGraphX(), me.getGraphY());
     ...
     */
    resizeVertex(me) {
        const ct = new Point_js_1.default(this.state.getCenterX(), this.state.getCenterY());
        const alpha = (0, mathUtils_js_1.toRadians)(this.state.style.rotation ?? 0);
        const point = new Point_js_1.default(me.getGraphX(), me.getGraphY());
        const tr = this.graph.view.translate;
        const { scale } = this.graph.view;
        ...
        if (geo && this.index !== null) {
            this.unscaledBounds = this.union(geo, dx / scale, dy / scale, this.index, this.graph.isGridEnabledEvent(me.getEvent()), 1, new Point_js_1.default(0, 0), this.isConstrainedEvent(me), this.isCenteredEvent(this.state, me));
        }
        ...
        if (this.unscaledBounds) {
            ...
            this.bounds = new Rectangle_js_1.default((this.parentState ? this.parentState.x : tr.x * scale) +
            ...
            cos = Math.cos(alpha);
            sin = Math.sin(alpha);
            const c2 = new Point_js_1.default(this.bounds.getCenterX(), this.bounds.getCenterY());
            dx = c2.x - ct.x;
            dy = c2.y - ct.y;
    ...
    updateLivePreview(me) {
        ...
        this.state.origin = new Point_js_1.default(this.state.x / scale - tr.x, this.state.y / scale - tr.y);
        this.state.width = this.bounds.width;
        this.state.height = this.bounds.height;
        // Redraws cell and handles
        let off = this.state.absoluteOffset;
        off = new Point_js_1.default(off.x, off.y);
    ...
    mouseUp(_sender, me) {
        if (this.index != null && this.state != null) {
            ...
            this.graph.batchUpdate(() => {
                if (index <= InternalEvent_js_1.default.CUSTOM_HANDLE) {
                    if (this.customHandles != null) {
                        // Creates style before changing cell state
                        const style = this.state.view.graph.getCellStyle(this.state.cell);
                        this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - index].active = false;
                        ...
                        if (this.customHandles != null &&
                            this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - index] != null) {
                            this.state.style = style;
                            this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - index].positionChanged();
                        }
                    }
                }
                else if (index === InternalEvent_js_1.default.ROTATION_HANDLE) {
            ...
            if (cell.isVertex() || cell.isEdge()) {
                ...
                if (geo && parent) {
                    ...
                    if (pgeo != null && !parent.isEdge()) {
                        geo = geo.clone();
                        geo.rotate(angle, new Point_js_1.default(pgeo.width / 2, pgeo.height / 2));
    ...
    resizeCell(cell, dx, dy, index, gridEnabled, constrained, recurse) {
        let geo = cell.getGeometry();
        if (geo) {
            if (index === InternalEvent_js_1.default.LABEL_HANDLE &&
                this.labelShape &&
                ...
                const sin = Math.sin(alpha);
                const { scale } = this.graph.view;
                const pt = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(Math.round((this.labelShape.bounds.getCenterX() - this.startX) / scale), Math.round((this.labelShape.bounds.getCenterY() - this.startY) / scale)), cos, sin);
                geo = geo.clone();
                if (geo.offset == null) {
                    geo.offset = pt;
                }
    ...
    union(bounds, dx, dy, index, gridEnabled, scale, tr, constrained, centered) {
        ...
        if (this.singleSizer) {
            ...
            const rect = new Rectangle_js_1.default(bounds.x, bounds.y, 0, 0);
            rect.add(new Rectangle_js_1.default(x, y, 0, 0));
            return rect;
        }
        ...
        const result = new Rectangle_js_1.default(left + tr.x * scale, top + tr.y * scale, width, height);
    ...
    getHandlePadding() {
        ...
        const result = new Point_js_1.default(0, 0);
    ...
    redrawHandles() {
        ...
        if (this.sizers.length > 0 && this.sizers[0]) {
            if (this.index === null && this.manageSizers && this.sizers.length >= 8) {
                ...
                if (this.horizontalOffset !== 0 || this.verticalOffset !== 0) {
                    s = new Rectangle_js_1.default(s.x, s.y, s.width, s.height);
            ...
            if (this.singleSizer) {
                ...
                if (this.sizers.length >= 8) {
                    ...
                    const ct = new Point_js_1.default(s.getCenterX(), s.getCenterY());
                    let pt = (0, mathUtils_js_1.getRotatedPoint)(new Point_js_1.default(s.x, s.y), cos, sin, ct);
                    this.moveSizerTo(this.sizers[0], pt.x, pt.y);
                    this.sizers[0].setCursor(crs[(0, mathUtils_js_1.mod)(0 + da, crs.length)]);
                    pt.x = cx;
                    pt.y = s.y;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[1], pt.x, pt.y);
                    this.sizers[1].setCursor(crs[(0, mathUtils_js_1.mod)(1 + da, crs.length)]);
                    pt.x = r;
                    pt.y = s.y;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[2], pt.x, pt.y);
                    this.sizers[2].setCursor(crs[(0, mathUtils_js_1.mod)(2 + da, crs.length)]);
                    pt.x = s.x;
                    pt.y = cy;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[3], pt.x, pt.y);
                    this.sizers[3].setCursor(crs[(0, mathUtils_js_1.mod)(7 + da, crs.length)]);
                    pt.x = r;
                    pt.y = cy;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[4], pt.x, pt.y);
                    this.sizers[4].setCursor(crs[(0, mathUtils_js_1.mod)(3 + da, crs.length)]);
                    pt.x = s.x;
                    pt.y = b;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[5], pt.x, pt.y);
                    this.sizers[5].setCursor(crs[(0, mathUtils_js_1.mod)(6 + da, crs.length)]);
                    pt.x = cx;
                    pt.y = b;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[6], pt.x, pt.y);
                    this.sizers[6].setCursor(crs[(0, mathUtils_js_1.mod)(5 + da, crs.length)]);
                    pt.x = r;
                    pt.y = b;
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, ct);
                    this.moveSizerTo(this.sizers[7], pt.x, pt.y);
                    this.sizers[7].setCursor(crs[(0, mathUtils_js_1.mod)(4 + da, crs.length)]);
        ...
        if (this.rotationShape) {
            const alpha = (0, mathUtils_js_1.toRadians)(this.currentAlpha ?? this.state.style.rotation ?? 0);
            const cos = Math.cos(alpha);
            const sin = Math.sin(alpha);
            const ct = new Point_js_1.default(this.state.getCenterX(), this.state.getCenterY());
            const pt = (0, mathUtils_js_1.getRotatedPoint)(this.getRotationHandlePosition(), cos, sin, ct);
    ...
    getRotationHandlePosition() {
        return new Point_js_1.default(this.bounds.x + this.bounds.width / 2, this.bounds.y + this.rotationHandleVSpacing);
    }
    /**
