...
const removeCursors = (element) => {
    ...
    if (children) {
        ...
        for (let i = 0; i < childCount; i += 1) {
            (0, exports.removeCursors)(children[i]);
        }
    }
};
...
const setPrefixedStyle = (style, name, value) => {
    let prefix = null;
    if (Client_js_1.default.IS_SF || Client_js_1.default.IS_GC) {
        prefix = 'Webkit';
    }
    else if (Client_js_1.default.IS_MT) {
        prefix = 'Moz';
    }
...
const hasScrollbars = (node) => {
    const style = (0, exports.getCurrentStyle)(node);
...
const getDocumentSize = () => {
    ...
    try {
        return new Rectangle_js_1.default(0, 0, b.clientWidth ?? d.clientWidth, Math.max(b.clientHeight ?? 0, d.clientHeight));
    }
    catch (e) {
        return new Rectangle_js_1.default();
    }
};
 ...
 */
const fit = (node) => {
    const ds = (0, exports.getDocumentSize)();
    const left = node.offsetLeft;
    const width = node.offsetWidth;
    const offset = (0, exports.getDocumentScrollOrigin)(node.ownerDocument);
    const sl = offset.x;
    const st = offset.y;
...
const getOffset = (container, scrollOffset = false) => {
    ...
    if (!scrollOffset && !fixed) {
        const offset = (0, exports.getDocumentScrollOrigin)(container.ownerDocument);
        offsetLeft += offset.x;
        offsetTop += offset.y;
    }
    ...
    return new Point_js_1.default(offsetLeft, offsetTop);
};
...
exports.getDocumentScrollOrigin = getDocumentScrollOrigin;
/**
...
const getScrollOrigin = (node = null, includeAncestors = false, includeDocument = true) => {
    ...
    const result = new Point_js_1.default();
    let fixed = false;
    ...
    if (!fixed && includeDocument) {
        const origin = (0, exports.getDocumentScrollOrigin)(doc);
        result.x += origin.x;
        result.y += origin.y;
    }
    return result;
};
exports.getScrollOrigin = getScrollOrigin;
/**
...
const convertPoint = (container, x, y) => {
    const origin = (0, exports.getScrollOrigin)(container, false);
    const offset = (0, exports.getOffset)(container);
    offset.x -= origin.x;
    offset.y -= origin.y;
    return new Point_js_1.default(x - offset.x, y - offset.y);
};
...
exports.setCellStyles = setCellStyles;
/**
    ...
    if (cells.length > 0) {
        model.batchUpdate(() => {
            for (let i = 0; i < cells.length; i += 1) {
                ...
                if (cell) {
                    // Currently, the style object must be cloned, otherwise model.setStyle does not trigger the change event and the cell state in the view is not updated
                    const style = (0, exports.setStyleFlag)(cell.getClonedStyle(), key, flag, value);
                    model.setStyle(cell, style);
                }
            }
        });
...
exports.setCellStyleFlags = setCellStyleFlags;
/**
...
exports.setStyleFlag = setStyleFlag;
/**
 ...
 */
const getSizeForString = (text, fontSize = Constants_js_1.DEFAULT_FONTSIZE, fontFamily = Constants_js_1.DEFAULT_FONTFAMILY, textWidth = null, fontStyle = null) => {
    const div = document.createElement('div');
    // Sets the font size and family
    div.style.fontFamily = fontFamily;
    div.style.fontSize = `${Math.round(fontSize)}px`;
    div.style.lineHeight = `${Math.round(fontSize * Constants_js_1.LINE_HEIGHT)}px`;
    // Sets the font style
    if (fontStyle !== null) {
        (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.BOLD) && (div.style.fontWeight = 'bold');
    ...
    const size = new Rectangle_js_1.default(0, 0, div.offsetWidth, div.offsetHeight);
    document.body.removeChild(div);
    return size;
};
exports.getSizeForString = getSizeForString;
/**
...
const sortCells = (cells, ascending = true) => {
    ...
    cells.sort((o1, o2) => {
        ...
        if (p1 == null) {
            p1 = CellPath_js_1.default.create(o1).split(CellPath_js_1.default.PATH_SEPARATOR);
            lookup.set(o1, p1);
        }
        let p2 = lookup.get(o2);
        if (p2 == null) {
            p2 = CellPath_js_1.default.create(o2).split(CellPath_js_1.default.PATH_SEPARATOR);
            lookup.set(o2, p2);
        }
        const comp = CellPath_js_1.default.compare(p1, p2);
