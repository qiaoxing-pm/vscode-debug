...
const BaseGraph_js_1 = require("../BaseGraph.js");
...
class Outline {
    ...
    init(container) {
        ...
        this.source.getDataModel().addListener(InternalEvent_js_1.default.CHANGE, this.updateHandler);
        this.outline.addMouseListener(this);
        // Adds listeners to keep the outline in sync with the source graph
        const view = this.source.getView();
        view.addListener(InternalEvent_js_1.default.SCALE, this.updateHandler);
        view.addListener(InternalEvent_js_1.default.TRANSLATE, this.updateHandler);
        view.addListener(InternalEvent_js_1.default.SCALE_AND_TRANSLATE, this.updateHandler);
        view.addListener(InternalEvent_js_1.default.DOWN, this.updateHandler);
        view.addListener(InternalEvent_js_1.default.UP, this.updateHandler);
        // Updates blue rectangle on scroll
        // @ts-ignore because sender and evt don't seem used
        InternalEvent_js_1.default.addListener(this.source.container, 'scroll', this.updateHandler);
        this.panHandler = (sender, evt) => {
            if (this.updateOnPan) {
                this.updateHandler(sender, evt);
            }
        };
        this.source.addListener(InternalEvent_js_1.default.PAN, this.panHandler);
        ...
        this.source.addListener(InternalEvent_js_1.default.REFRESH, this.refreshHandler);
        // Creates the blue rectangle for the viewport
        this.bounds = new Rectangle_js_1.default(0, 0, 0, 0);
        this.selectionBorder = new RectangleShape_js_1.default(this.bounds, Constants_js_1.NONE, Constants_js_1.OUTLINE_COLOR, Constants_js_1.OUTLINE_STROKEWIDTH);
        ...
        const handler = (evt) => {
            const t = (0, EventUtils_js_1.getSource)(evt);
            const redirect = (evt) => {
                const outline = this.outline;
                outline?.fireMouseEvent(InternalEvent_js_1.default.MOUSE_MOVE, new InternalMouseEvent_js_1.default(evt));
            };
            const redirect2 = (evt) => {
                const outline = this.outline;
                InternalEvent_js_1.default.removeGestureListeners(t, null, redirect, redirect2);
                outline?.fireMouseEvent(InternalEvent_js_1.default.MOUSE_UP, new InternalMouseEvent_js_1.default(evt));
            };
            const outline = this.outline;
            InternalEvent_js_1.default.addGestureListeners(t, null, redirect, redirect2);
            outline?.fireMouseEvent(InternalEvent_js_1.default.MOUSE_DOWN, new InternalMouseEvent_js_1.default(evt));
        };
        InternalEvent_js_1.default.addGestureListeners(this.selectionBorder.node, handler);
        ...
        InternalEvent_js_1.default.addGestureListeners(this.sizer.node, handler);
    ...
    createGraph(container) {
        ...
        const graph = new BaseGraph_js_1.BaseGraph({
            container,
            ...
            plugins: (0, index_js_1.getDefaultPlugins)(),
    ...
    createSizer() {
        ...
        if (this.sizerImage != null) {
            const sizer = new ImageShape_js_1.default(new Rectangle_js_1.default(0, 0, this.sizerImage.width, this.sizerImage.height), this.sizerImage.src);
            outline && (sizer.dialect = outline.dialect);
            return sizer;
        }
        const sizer = new RectangleShape_js_1.default(new Rectangle_js_1.default(0, 0, this.sizerSize, this.sizerSize), Constants_js_1.OUTLINE_HANDLE_FILLCOLOR, Constants_js_1.OUTLINE_HANDLE_STROKECOLOR);
        outline && (sizer.dialect = outline.dialect);
        return sizer;
    }
    /**
    ...
    getSourceContainerSize() {
        return new Rectangle_js_1.default(0, 0, this.source.container.scrollWidth, this.source.container.scrollHeight);
    }
    /**
    ...
    update(revalidate = false) {
        if (this.source != null &&
            ...
            const unscaledGraphBounds = new Rectangle_js_1.default(scaledGraphBounds.x / sourceScale + this.source.panDx, scaledGraphBounds.y / sourceScale + this.source.panDy, scaledGraphBounds.width / sourceScale, scaledGraphBounds.height / sourceScale);
            const unscaledFinderBounds = new Rectangle_js_1.default(0, 0, this.source.container.clientWidth / sourceScale, this.source.container.clientHeight / sourceScale);
            ...
            if (scale > 0) {
                ...
                this.bounds = new Rectangle_js_1.default((t2.x - t.x - this.source.panDx) / scale3, (t2.y - t.y - this.source.panDy) / scale3, container.clientWidth / scale2, container.clientHeight / scale2);
                ...
                b = sizer.bounds;
                const b2 = new Rectangle_js_1.default(this.bounds.x + this.bounds.width - b.width / 2, this.bounds.y + this.bounds.height - b.height / 2, b.width, b.height);
                if (b.x !== b2.x ||
                    b.y !== b2.y ||
                    b.width !== b2.width ||
                    b.height !== b2.height) {
    ...
    mouseDown(_sender, me) {
        if (this.enabled && this.showViewport) {
            const tol = !(0, EventUtils_js_1.isMouseEvent)(me.getEvent()) ? this.source.tolerance : 0;
            const hit = tol > 0
            ...
            if (this.source.useScrollbarsForPanning && (0, styleUtils_js_1.hasScrollbars)(this.source.container)) {
    ...
    mouseMove(_sender, me) {
        if (this.active) {
            ...
            if (outline && !this.zoom) {
                ...
                bounds = new Rectangle_js_1.default(myBounds.x + dx, myBounds.y + dy, myBounds.width, myBounds.height);
                ...
                dy = dx / viewRatio;
                bounds = new Rectangle_js_1.default(myBounds.x, myBounds.y, Math.max(1, myBounds.width + dx), Math.max(1, myBounds.height + dy));
                selectionBorder.bounds = bounds;
                selectionBorder.redraw();
            }
            // Updates the zoom handle
            const b = sizer.bounds;
            sizer.bounds = new Rectangle_js_1.default(bounds.x + bounds.width - b.width / 2, bounds.y + bounds.height - b.height / 2, b.width, b.height);
            // Avoids update of visibility in redraw for VML
    ...
    getTranslateForEvent(me) {
        return new Point_js_1.default(me.getX() - this.startX, me.getY() - this.startY);
    }
    /**
        ...
        if (this.active) {
            ...
            if (Math.abs(dx) > 0 || Math.abs(dy) > 0) {
                if (!this.zoom) {
                    ...
                    // has no scrollbars
                    if (outline &&
                        (!source.useScrollbarsForPanning || !(0, styleUtils_js_1.hasScrollbars)(source.container))) {
                        source.panGraph(0, 0);
    ...
    destroy() {
        if (this.source != null) {
            ...
            // @ts-ignore
            InternalEvent_js_1.default.removeListener(this.source.container, 'scroll', this.updateHandler);
            // @ts-ignore
            this.source = null;
        }
