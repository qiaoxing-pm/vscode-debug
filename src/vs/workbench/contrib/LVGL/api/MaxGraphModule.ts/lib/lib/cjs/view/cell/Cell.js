...
const CellPath_js_1 = __importDefault(require("./CellPath.js"));
...
class Cell {
     ...
     */
    getClonedStyle() {
        return (0, cloneUtils_js_1.clone)(this.getStyle());
    }
    /**
    ...
    hasAttribute(name) {
        ...
        return (0, utils_js_1.isElement)(userObject) && userObject.hasAttribute
            ? userObject.hasAttribute(name)
            : !(0, utils_js_1.isNullish)(userObject.getAttribute?.(name));
    }
    /**
    ...
    getAttribute(name, defaultValue) {
        const userObject = this.getValue();
        const val = (0, utils_js_1.isElement)(userObject) ? userObject.getAttribute?.(name) : null;
    ...
    setAttribute(name, value) {
        const userObject = this.getValue();
        if ((0, utils_js_1.isElement)(userObject)) {
     ...
     */
    clone() {
        const c = (0, cloneUtils_js_1.clone)(this, this.mxTransient);
    ...
    cloneValue() {
        ...
        if (!(0, utils_js_1.isNullish)(value)) {
            ...
            else if (!(0, utils_js_1.isNullish)(value.nodeType) && value.cloneNode) {
    ...
    getNearestCommonAncestor(cell2) {
        ...
        let path = CellPath_js_1.default.create(cell2);
        if (path.length > 0) {
            ...
            let cell = this;
            let current = CellPath_js_1.default.create(cell);
            // Inverts arguments
            ...
            while (cell && current) {
                ...
                if (path.indexOf(current + CellPath_js_1.default.PATH_SEPARATOR) === 0 && parent) {
                    return cell;
                }
                current = CellPath_js_1.default.getParentPath(current);
                cell = parent;
            }
        }
        return null;
     ...
     */
    getOrigin() {
        let result = new Point_js_1.default();
