...
import mxGuide from '../other/Guide.js';
...
import type { ColorValue, GraphPlugin } from '../../types.js';
...
class SelectionHandler implements GraphPlugin {
	...
	updatePreviewShape() {
		if (this.shape && this.pBounds) {
			this.graph.fireEvent(new EventObject("CELL_MOVING", {
				cell: this.cell,
				dx: this.currentDx / this.graph.view.scale,
				dy: this.currentDy / this.graph.view.scale
			}));
