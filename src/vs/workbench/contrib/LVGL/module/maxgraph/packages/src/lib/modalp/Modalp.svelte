<script lang="ts">
  ...
  import { type ModalpProps as Props, modalp } from ".";
  import { fade } from "svelte/transition";
  import { sineIn } from "svelte/easing";

  let { children, header, footer, title, modalStatus, dismissable = true, left, top, closeModal, divClass, contentClass, closeBtnClass, h3Class, headerClass, bodyClass, footerClass, outsideClose = true, size = "md", backdrop = true, backdropClass, position = "center", class: className, params = { duration: 100, easing: sineIn }, transition = fade, rounded = true, ...restProps }: Props = $props();
...
{#if modalStatus}
  ...
  {/if}
  <div {...restProps} class={base({ className })} style:left={left} style:top={top} transition:transition={params as ParamsType} tabindex="-1">
