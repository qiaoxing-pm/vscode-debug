...
import MaxWindow, { error } from '../gui/MaxWindow.js';
...
import { ALIGN, FONT } from '../util/Constants.js';
...
import mxClipboard from '../util/Clipboard.js';
import MaxLog from '../gui/MaxLog.js';
import { isNode } from '../util/domUtils.js';
import { getViewXml, getXml } from '../util/xmlUtils.js';
import { load, post, submit } from '../util/MaxXmlRequest.js';
import PopupMenuHandler from '../view/handler/PopupMenuHandler.js';
import RubberBandHandler from '../view/handler/RubberBandHandler.js';
import InternalEvent from '../view/event/InternalEvent.js';
import InternalMouseEvent from '../view/event/InternalMouseEvent.js';
import type { CellStateStyle, MouseListenerSet } from '../types.js';
import ConnectionHandler from '../view/handler/ConnectionHandler.js';
import { show } from '../util/printUtils.js';
import PanningHandler from '../view/handler/PanningHandler.js';
...
export class Editor extends EventSource {
	 ...
	 * @example
	 * ```javascript
	 * editor.addAction('iconfont', function(editor: Editor, cell: Cell)
	 * {
	 * 		mxUtils.alert("iconfont "+cell);
	 * });
	 * ```
	...
	setStatusContainer(container: any): void {
		if (this.status == null) {
			...
			this.addListener(InternalEvent.OPEN, () => {
				this.setStatus(
					`${Translations.get(this.currentFileResource) || this.currentFileResource}: ${this.filename
					}`
				);
			});
		}
	...
	createProperties(cell: Cell): HTMLTableElement | null {
		...
		if (isNode(value)) {
			...
			const okFunction = () => {
				...
				// XML StructureEdit / XML node attribute changes.
				model.beginUpdate();
				try {
					if (
						geo != null &&
