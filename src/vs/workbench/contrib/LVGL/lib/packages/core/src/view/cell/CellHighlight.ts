...
class CellHighlight {
	...
	createShape() {
		...
		let cellShape = this.state.style.shape;
		let shape: Shape | null = null;
		if (cellShape?.startsWith('lvgl')) {
			let ctor = this.graph.cellRenderer.getShape("rectangle")!;
			shape = new ctor();
		} else {
			shape = this.graph.cellRenderer.createShape(this.state);
		}
