...
import ConnectorShape from '../geometry/node/ConnectorShape.js';
...
class ConstraintHandler {
	 ...
	 */
	pointImage = new Image(`${Client.imageBasePath}/point.gif`, 5, 20);
	...
	focusIcons: ConnectorShape[] = [];
	frames: RectangleShape[] = [];
	...
	destroyFocusHighlight() {
		if (this.focusHighlight) {
			this.focusHighlight.destroy();
			this.focusHighlight = null;
			this.graph.fireEvent(new EventObject("destroyConnector", null));
		}
	}
			...
			if (
				...
				for (let i = 0; i < this.focusIcons.length; i += 1) {
					...
					if (
						...
						tmp = this.focusIcons[i].iconBounds!.clone();
	...
	setFocus(me: InternalMouseEvent, state: CellState | null, source: boolean) {
		...
		if (this.constraints && state) {
			...
			this.focusIcons = [];
			this.focusPoints = [];
			const connectors = this.constraints as Connector[];
			for (let i = 0; i < this.constraints.length; i += 1) {
				const cp = this.graph.getConnectionPoint(state, this.constraints[i]) as Point;
				// 这段是对源码进行添加
				...
				const icon = new ConnectorShape(connectors[i]);
				icon.dialect =
				...
				InternalEvent.redirectMouseEvents(icon.node, this.graph, getState, (e: MouseEvent) => {
					console.log("click");
					this.graph.fireEvent(new EventObject("showConnector", {
						event: e, connector: connectors[i], bounds: connectors[i].iconBounds!!
					}));
				});
