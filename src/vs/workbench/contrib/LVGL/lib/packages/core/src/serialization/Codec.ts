...
class Codec {
	...
	decode(node: Element | null, into?: any): any {
		...
		if (node != null && node.nodeType === NODETYPE.ELEMENT) {
			...
			if (name == 'interface') name = 'Interface'
			// const dec = CodecRegistry.getCodecByName(node.nodeName);
			const dec = CodecRegistry.getCodecByName(name);
			if (dec != null) {
	 ...
	 * and restores the group and graph StructureEdit in the cells.
	 ...
	 * the graph StructureEdit should be restored by calling insert
