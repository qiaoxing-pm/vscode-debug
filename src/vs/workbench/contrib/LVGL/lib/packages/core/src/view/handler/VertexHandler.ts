...
import type { CellHandle, Listenable } from '../../types.js';
...
import { VertexHandlerConfig } from './config.js';
...
class VertexHandler {
	...
	resizeVertex(me: InternalMouseEvent) {
		...
		if (this.unscaledBounds) {
			...
			let offset = graph.getChildOffsetForCell(this.state.cell);
			if (offset == null) {
				offset = new Point(0, 0);
			}
			this.bounds = new Rectangle(
				(this.parentState ? this.parentState.x : tr.x * scale) + offset.x * scale +
				this.unscaledBounds.x * scale,
				(this.parentState ? this.parentState.y : tr.y * scale) + offset.y * scale +
	...
	drawPreview() {
		if (this.preview != null) {
			this.graph.fireEvent(new EventObject("CELL_RESIZING", {
				width: this.bounds.width / this.graph.view.scale,
				height: this.bounds.height / this.graph.view.scale,
			}));
