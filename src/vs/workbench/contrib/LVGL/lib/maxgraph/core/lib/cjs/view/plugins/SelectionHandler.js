...
const Guide_js_1 = __importDefault(require("../other/Guide.js"));
...
const config_js_1 = require("../handler/config.js");
/**
...
class SelectionHandler {
    ...
    constructor(graph) {
         ...
         */
        this.allowLivePreview = Client_js_1.default.IS_SVG;
        this.cell = null;
        ...
        this.graph.addListener(InternalEvent_js_1.default.PAN, this.panHandler);
        ...
        this.graph.getDataModel().addListener(InternalEvent_js_1.default.CHANGE, this.refreshHandler);
        this.graph.addListener(InternalEvent_js_1.default.REFRESH, this.refreshHandler);
        this.keyHandler = (e) => {
        ...
        if (typeof document !== 'undefined') {
            InternalEvent_js_1.default.addListener(document, 'keydown', this.keyHandler);
            InternalEvent_js_1.default.addListener(document, 'keyup', this.keyHandler);
        }
    }
    /**
     * Returns <enabled>.
    ...
    getInitialCellForEvent(me) {
        ...
        if ((!this.graph.isToggleEvent(me.getEvent()) || !(0, EventUtils_js_1.isAltDown)(me.getEvent())) &&
            state &&
    ...
    isDelayedSelection(cell, me) {
        ...
        return this.graph.isToggleEvent(me.getEvent()) && !(0, EventUtils_js_1.isAltDown)(me.getEvent());
    }
    /**
        ...
        if (!me.isConsumed() &&
            ...
            if (cell) {
                ...
                if (this.isMoveEnabled()) {
                    ...
                    this.consumeMouseEvent(InternalEvent_js_1.default.MOUSE_DOWN, me);
                }
            }
        }
    }
    ...
    createPreviewShape(bounds) {
        const shape = new RectangleShape_js_1.default(bounds, Constants_js_1.NONE, this.previewColor);
        shape.isDashed = true;
        if (this.htmlPreview) {
    ...
    createGuide() {
        return new Guide_js_1.default(this.graph, this.getGuideStates());
    }
    /**
    ...
    start(cell, x, y, cells) {
        this.cell = cell;
        this.first = (0, styleUtils_js_1.convertPoint)(this.graph.container, x, y);
     ...
     */
    getDelta(me) {
        const point = (0, styleUtils_js_1.convertPoint)(this.graph.container, me.getX(), me.getY());
        if (!this.first)
            return new Point_js_1.default();
        return new Point_js_1.default(point.x - this.first.x - this.graph.getPanDx(), point.y - this.first.y - this.graph.getPanDy());
    }
    /**
    ...
    mouseMove(_sender, me) {
        ...
        if (!me.isConsumed() &&
            ...
            if ((0, EventUtils_js_1.isMultiTouchEvent)(me.getEvent())) {
                this.reset();
                return;
            }
            ...
            if (this.shape ||
                ...
                if (!this.highlight) {
                    this.highlight = new CellHighlight_js_1.default(this.graph, Constants_js_1.DROP_TARGET_COLOR, 3);
                }
                ...
                if (state && (clone || (target && this.isValidDropTarget(target, me)))) {
                    if (this.target !== target) {
                        this.target = target;
                        this.setHighlightColor(Constants_js_1.DROP_TARGET_COLOR);
                    }
                    highlight = true;
                }
                else {
                    ...
                    if (this.connectOnDrop &&
                        ...
                        if (state) {
                            ...
                            const color = error === null ? Constants_js_1.VALID_COLOR : Constants_js_1.INVALID_CONNECT_TARGET_COLOR;
            ...
            this.updateHint(me);
            this.consumeMouseEvent(InternalEvent_js_1.default.MOUSE_MOVE, me);
            ...
            if (!cursor && cell && graph.isEnabled() && graph.isCellMovable(cell)) {
                if (cell.isEdge()) {
                    cursor = config_js_1.EdgeHandlerConfig.cursorMovable;
                }
                else {
                    cursor = config_js_1.VertexHandlerConfig.cursorMovable;
                }
            }
    ...
    updatePreviewShape() {
        if (this.shape && this.pBounds) {
            this.shape.bounds = new Rectangle_js_1.default(Math.round(this.pBounds.x + this.currentDx), Math.round(this.pBounds.y + this.currentDy), this.pBounds.width, this.pBounds.height);
            this.shape.redraw();
        }
    }
    /**
                ...
                for (let i = 0; i < states.length; i += 1) {
                    ...
                    if (state.cell.isEdge()) {
                        ...
                        if (geometry && geometry.points) {
                            for (let j = 0; j < geometry.points.length; j++) {
                                if (geometry.points[j]) {
                                    points.push(new Point_js_1.default(geometry.points[j].x + dx / s, geometry.points[j].y + dy / s));
                                }
                            }
                        }
                        ...
                        if (source == null || !this.isCellMoving(source.cell)) {
                            const pt0 = pts[0];
                            if (pt0) {
                                state.setAbsoluteTerminalPoint(new Point_js_1.default(pt0.x + dx, pt0.y + dy), true);
                                source = null;
                            }
                        }
                        else {
                        ...
                        if (target == null || !this.isCellMoving(target.cell)) {
                            ...
                            if (ptn) {
                                state.setAbsoluteTerminalPoint(new Point_js_1.default(ptn.x + dx, ptn.y + dy), false);
                                target = null;
                            }
                        }
                        else {
    ...
    mouseUp(_sender, me) {
        if (!me.isConsumed()) {
            ...
            if (this.cell &&
                ...
                (0, mathUtils_js_1.isNumeric)(this.currentDx) &&
                (0, mathUtils_js_1.isNumeric)(this.currentDy)) {
                const { graph } = this;
                const cell = me.getCell();
                if (this.connectOnDrop &&
                    !this.target &&
        ...
        if (this.cellWasClicked) {
            this.consumeMouseEvent(InternalEvent_js_1.default.MOUSE_UP, me);
        }
        this.reset();
    }
    /**
    ...
    shouldRemoveCellsFromParent(parent, cells, evt) {
        if (parent.isVertex()) {
            ...
            if (pState) {
                let pt = (0, styleUtils_js_1.convertPoint)(this.graph.container, (0, EventUtils_js_1.getClientX)(evt), (0, EventUtils_js_1.getClientY)(evt));
                ...
                if (alpha !== 0) {
                    ...
                    pt = (0, mathUtils_js_1.getRotatedPoint)(pt, cos, sin, cx);
                }
                return !(0, mathUtils_js_1.contains)(pState, pt.x, pt.y);
            }
        }
        return false;
    }
    ...
    onDestroy() {
        ...
        InternalEvent_js_1.default.removeListener(document, 'keydown', this.keyHandler);
        InternalEvent_js_1.default.removeListener(document, 'keyup', this.keyHandler);
        this.destroyShapes();
        this.removeHint();
    }
