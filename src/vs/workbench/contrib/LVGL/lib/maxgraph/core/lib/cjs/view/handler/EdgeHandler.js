...
const CellMarker_js_1 = __importDefault(require("../cell/CellMarker.js"));
...
const EllipseShape_js_1 = __importDefault(require("../shape/node/EllipseShape.js"));
const ImageShape_js_1 = __importDefault(require("../shape/node/ImageShape.js"));
const RectangleShape_js_1 = __importDefault(require("../shape/node/RectangleShape.js"));
const ConnectionConstraint_js_1 = __importDefault(require("../other/ConnectionConstraint.js"));
const InternalEvent_js_1 = __importDefault(require("../event/InternalEvent.js"));
const ConstraintHandler_js_1 = __importDefault(require("./ConstraintHandler.js"));
const Rectangle_js_1 = __importDefault(require("../geometry/Rectangle.js"));
const Client_js_1 = __importDefault(require("../../Client.js"));
const builtin_style_elements_js_1 = require("../style/builtin-style-elements.js");
...
class EdgeHandler {
    constructor(state) {
        ...
        this.shape.setCursor(config_js_1.EdgeHandlerConfig.cursorMovable);
        InternalEvent_js_1.default.redirectMouseEvents(this.shape.node, this.graph, this.state);
        // Updates preferHtml
        this.preferHtml =
        ...
        this.label = new Point_js_1.default(this.state.absoluteOffset.x, this.state.absoluteOffset.y);
        this.labelShape = this.createLabelHandleShape();
        this.initBend(this.labelShape);
        this.labelShape.setCursor(config_js_1.HandleConfig.labelCursor);
        ...
        this.state.view.graph.addListener(InternalEvent_js_1.default.ESCAPE, this.escapeHandler);
    }
    /**
    ...
    createConstraintHandler() {
        return new ConstraintHandler_js_1.default(this.graph);
    }
    /**
            ...
            if (this.parentHighlight) {
                if (parent && parent.isVertex() && visible) {
                    ...
                    if (pstate &&
                        ...
                        this.parentHighlight.bounds = Rectangle_js_1.default.fromRectangle(pstate);
    ...
    isVirtualBendsEnabled(evt) {
        return (config_js_1.EdgeHandlerConfig.virtualBendsEnabled &&
            (this.state.style.edgeStyle == null ||
                this.state.style.edgeStyle === Constants_js_1.NONE ||
     ...
     */
    isAddPointEvent(evt) {
        return (0, EventUtils_js_1.isShiftDown)(evt);
    }
    /**
    ...
    createParentHighlightShape(bounds) {
        const shape = new RectangleShape_js_1.default(Rectangle_js_1.default.fromRectangle(bounds), Constants_js_1.NONE, this.getSelectionColor());
     ...
     */
    getSelectionColor() {
        return config_js_1.EdgeHandlerConfig.selectionColor;
    }
    /**
     * Returns {@link EdgeHandlerConfig.selectionStrokeWidth}.
     */
    getSelectionStrokeWidth() {
        return config_js_1.EdgeHandlerConfig.selectionStrokeWidth;
    }
    /**
     * Returns {@link EdgeHandlerConfig.selectionDashed}.
     */
    isSelectionDashed() {
        return config_js_1.EdgeHandlerConfig.selectionDashed;
    }
    /**
                ...
                if (terminal || this.graph.isCellBendable(cell)) {
                    ((index) => {
                        ...
                        if (this.isHandleEnabled(i)) {
                            bend.setCursor(terminal ? config_js_1.EdgeHandlerConfig.cursorTerminal : config_js_1.EdgeHandlerConfig.cursorBend);
                        }
                        bends.push(bend);
                        if (!terminal) {
    ...
    createVirtualBends() {
        ...
        if (this.graph.isCellBendable(cell)) {
            for (let i = 1; i < this.abspoints.length; i += 1) {
                ((bend) => {
                    this.initBend(bend);
                    bend.setCursor(config_js_1.EdgeHandlerConfig.cursorVirtualBend);
                    bends.push(bend);
    ...
    isHandleVisible(index) {
        ...
        const edgeStyle = geo
            ...
            : null;
        return (edgeStyle !== builtin_style_elements_js_1.EdgeStyle.EntityRelation ||
            index === 0 ||
    ...
    createHandleShape(_index) {
        if (this.handleImage) {
            const shape = new ImageShape_js_1.default(new Rectangle_js_1.default(0, 0, this.handleImage.width, this.handleImage.height), this.handleImage.src);
            // Allows HTML rendering of the images
            shape.preserveImageAspect = false;
            return shape;
        }
        let s = config_js_1.HandleConfig.size;
        if (this.preferHtml) {
            s -= 1;
        }
        const shapeConstructor = config_js_1.EdgeHandlerConfig.handleShape === 'circle' ? EllipseShape_js_1.default : RectangleShape_js_1.default;
        return new shapeConstructor(new Rectangle_js_1.default(0, 0, s, s), config_js_1.HandleConfig.fillColor, config_js_1.HandleConfig.strokeColor);
    }
    /**
    ...
    createLabelHandleShape() {
        if (this.labelHandleImage) {
            const shape = new ImageShape_js_1.default(new Rectangle_js_1.default(0, 0, this.labelHandleImage.width, this.labelHandleImage.height), this.labelHandleImage.src);
            // Allows HTML rendering of the images
            shape.preserveImageAspect = false;
            return shape;
        }
        const s = config_js_1.HandleConfig.labelSize;
        return new RectangleShape_js_1.default(new Rectangle_js_1.default(0, 0, s, s), config_js_1.HandleConfig.labelFillColor, config_js_1.HandleConfig.strokeColor);
    }
    /**
    ...
    initBend(bend, dblClick) {
        ...
        InternalEvent_js_1.default.redirectMouseEvents(bend.node, this.graph, this.state, null, null, null, dblClick);
    ...
    getHandleForEvent(me) {
        ...
        const tol = !(0, EventUtils_js_1.isMouseEvent)(me.getEvent()) ? this.tolerance : 1;
        const hit = this.allowHandleBoundsCheck && tol > 0
            ? new Rectangle_js_1.default(me.getGraphX() - tol, me.getGraphY() - tol, 2 * tol, 2 * tol)
            : null;
        ...
        function checkShape(shape) {
            if (shape &&
                ...
                (me.isSource(shape) || (hit && (0, mathUtils_js_1.intersects)(shape.bounds, hit)))) {
        ...
        if (this.isCustomHandleEvent(me) && this.customHandles) {
            ...
            for (let i = this.customHandles.length - 1; i >= 0; i--) {
                if (checkShape(this.customHandles[i].shape)) {
                    ...
                    return InternalEvent_js_1.default.CUSTOM_HANDLE - i;
                }
            }
        }
        if (me.isSource(this.state.text) || checkShape(this.labelShape)) {
            result = InternalEvent_js_1.default.LABEL_HANDLE;
        }
        ...
        if (this.virtualBends && this.isAddVirtualBendEvent(me)) {
            for (let i = 0; i < this.virtualBends.length; i += 1) {
                if (checkShape(this.virtualBends[i])) {
                    result = InternalEvent_js_1.default.VIRTUAL_HANDLE - i;
                }
            }
        }
        return result;
    ...
    mouseDown(_sender, me) {
        ...
        if (handle !== null && this.bends[handle]) {
            ...
            if (b)
                this.snapPoint = new Point_js_1.default(b.getCenterX(), b.getCenterY());
        }
        if (config_js_1.EdgeHandlerConfig.addBendOnShiftClickEnabled &&
            handle === null &&
            ...
            if (config_js_1.EdgeHandlerConfig.removeBendOnShiftClickEnabled &&
                this.isRemovePointEvent(me.getEvent())) {
                this.removePoint(this.state, handle);
            }
            else if (handle !== InternalEvent_js_1.default.LABEL_HANDLE ||
                (cell && this.graph.isLabelMovable(cell))) {
                if (this.virtualBends && handle <= InternalEvent_js_1.default.VIRTUAL_HANDLE) {
                    (0, styleUtils_js_1.setOpacity)(this.virtualBends[InternalEvent_js_1.default.VIRTUAL_HANDLE - handle].node, 100);
                }
    ...
    start(x, y, index) {
        ...
        this.isLabel = index === InternalEvent_js_1.default.LABEL_HANDLE;
        ...
        if (this.index !== null &&
            this.index <= InternalEvent_js_1.default.CUSTOM_HANDLE &&
            this.index > InternalEvent_js_1.default.VIRTUAL_HANDLE) {
            if (this.customHandles != null) {
                for (let i = 0; i < this.customHandles.length; i += 1) {
                    if (i !== InternalEvent_js_1.default.CUSTOM_HANDLE - this.index) {
    ...
    isSnapToTerminalsEvent(me) {
        return this.snapToTerminals && !(0, EventUtils_js_1.isAltDown)(me.getEvent());
    }
    /**
    ...
    getPointForEvent(me) {
        ...
        const { scale } = view;
        const point = new Point_js_1.default(this.roundLength(me.getGraphX() / scale) * scale, this.roundLength(me.getGraphY() / scale) * scale);
        ...
        if (tt > 0 && this.isSnapToTerminalsEvent(me)) {
            ...
            const snapToTerminal = (terminal) => {
                if (terminal) {
                    snapToPoint(new Point_js_1.default(view.getRoutingCenterX(terminal), view.getRoutingCenterY(terminal)));
                }
            };
    ...
    getPreviewPoints(pt, me) {
        ...
        const point = new Point_js_1.default(pt.x, pt.y);
        let result = null;
        if (!this.isSource && !this.isTarget && this.index !== null) {
            this.convertPoint(point, false);
            // Adds point from virtual bend
            if (this.index <= InternalEvent_js_1.default.VIRTUAL_HANDLE) {
                points.splice(InternalEvent_js_1.default.VIRTUAL_HANDLE - this.index, 0, point);
            }
            ...
            if (!this.isSource && !this.isTarget) {
                for (let i = 0; i < this.bends.length; i += 1) {
                    if (i !== this.index) {
                        ...
                        if (bend && (0, mathUtils_js_1.contains)(bend.bounds, pt.x, pt.y)) {
                            if (this.index <= InternalEvent_js_1.default.VIRTUAL_HANDLE) {
                                points.splice(InternalEvent_js_1.default.VIRTUAL_HANDLE - this.index, 1);
                            }
                            else {
                ...
                if (!result && this.straightRemoveEnabled && (!me || !(0, EventUtils_js_1.isAltDown)(me.getEvent()))) {
                    ...
                    if (src != null) {
                        ...
                        if (c == null || this.graph.getConnectionPoint(src, c) == null) {
                            abs[0] = new Point_js_1.default(src.view.getRoutingCenterX(src), src.view.getRoutingCenterY(src));
                        }
                    }
                    ...
                    if (trg != null) {
                        ...
                        if (c == null || this.graph.getConnectionPoint(trg, c) == null) {
                            abs[abs.length - 1] = new Point_js_1.default(trg.view.getRoutingCenterX(trg), trg.view.getRoutingCenterY(trg));
                        }
                    }
                    const checkRemove = (idx, tmp) => {
                        if (idx > 0 &&
                            idx < abs.length - 1 &&
                            (0, mathUtils_js_1.ptSegDistSq)(abs[idx - 1].x, abs[idx - 1].y, abs[idx + 1].x, abs[idx + 1].y, tmp.x, tmp.y) < tol) {
            ...
            if (result == null && this.index > InternalEvent_js_1.default.VIRTUAL_HANDLE) {
    ...
    isOutlineConnectEvent(me) {
        if (!this.currentPoint)
            return false;
        const offset = (0, styleUtils_js_1.getOffset)(this.graph.container);
        const evt = me.getEvent();
        const clientX = (0, EventUtils_js_1.getClientX)(evt);
        const clientY = (0, EventUtils_js_1.getClientY)(evt);
        ...
        return (this.outlineConnect &&
            !(0, EventUtils_js_1.isShiftDown)(me.getEvent()) &&
            (me.isSource(this.marker.highlight.shape) ||
                ((0, EventUtils_js_1.isAltDown)(me.getEvent()) && me.getState() != null) ||
        ...
        if (constraint == null && outline) {
            if (terminalState != null) {
                ...
                if (me.isSource(this.marker.highlight.shape)) {
                    point = new Point_js_1.default(me.getGraphX(), me.getGraphY());
                }
                ...
                constraint = new ConnectionConstraint_js_1.default(null);
            }
        }
        if (this.outlineConnect &&
            ...
            if (this.constraintHandler.currentConstraint != null &&
                ...
                this.marker.highlight.shape.stroke = outline
                    ? Constants_js_1.OUTLINE_HIGHLIGHT_COLOR
                    : 'transparent';
                this.marker.highlight.shape.strokeWidth = Constants_js_1.OUTLINE_HIGHLIGHT_STROKEWIDTH / s / s;
                this.marker.highlight.repaint();
            }
                ...
                this.marker.highlight.shape.stroke =
                    cell && cell.isConnectable() && this.marker.getValidState() !== me.getState()
                        ? 'transparent'
                        : Constants_js_1.DEFAULT_VALID_COLOR;
                this.marker.highlight.shape.strokeWidth = Constants_js_1.HIGHLIGHT_STROKEWIDTH / s / s;
                this.marker.highlight.repaint();
            }
        }
        if (this.isSource) {
    ...
    mouseMove(_sender, me) {
        if (this.index != null && this.marker != null) {
            ...
            if (this.index <= InternalEvent_js_1.default.CUSTOM_HANDLE &&
                this.index > InternalEvent_js_1.default.VIRTUAL_HANDLE) {
                if (this.customHandles != null) {
                    this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - this.index].processEvent(me);
                    this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - this.index].positionChanged();
            ...
            this.drawPreview();
            InternalEvent_js_1.default.consume(me.getEvent());
            me.consume();
        }
    }
    /**
    ...
    mouseUp(_sender, me) {
        ...
        if (this.index != null && this.marker != null) {
            ...
            if (me.getX() !== this.startX || me.getY() !== this.startY) {
                ...
                else if (index <= InternalEvent_js_1.default.CUSTOM_HANDLE &&
                    index > InternalEvent_js_1.default.VIRTUAL_HANDLE) {
                    if (this.customHandles != null) {
                        const model = this.graph.getDataModel();
                        model.beginUpdate();
                        try {
                            this.customHandles[InternalEvent_js_1.default.CUSTOM_HANDLE - index].execute(me);
                            if (this.shape != null && this.shape.node != null) {
    ...
    reset() {
        ...
        this.setPreviewColor(config_js_1.EdgeHandlerConfig.selectionColor);
        this.removeHint();
        this.redraw();
    }
    /**
    ...
    moveLabel(edgeState, x, y) {
        ...
        if (geometry != null) {
            ...
            if (geometry.relative) {
                ...
                geometry.offset = new Point_js_1.default(0, 0);
                pt = this.graph.view.getPoint(edgeState, geometry);
                geometry.offset = new Point_js_1.default(Math.round((x - pt.x) / scale), Math.round((y - pt.y) / scale));
            }
            else {
                ...
                if (p0 != null && pe != null) {
                    ...
                    geometry.offset = new Point_js_1.default(Math.round((x - cx) / scale), Math.round((y - cy) / scale));
                    geometry.x = 0;
                    geometry.y = 0;
                }
            }
    ...
    changeTerminalPoint(edge, point, isSource, clone) {
        ...
        model.batchUpdate(() => {
            ...
            if (geo != null) {
                ...
                this.graph.connectCell(edge, null, isSource, new ConnectionConstraint_js_1.default(null));
            }
        });
        return edge;
    }
     ...
     */
    addPoint(state, evt) {
        const pt = (0, styleUtils_js_1.convertPoint)(this.graph.container, (0, EventUtils_js_1.getClientX)(evt), (0, EventUtils_js_1.getClientY)(evt));
    ...
    addPointAt(state, x, y) {
        ...
        const pt = new Point_js_1.default(x, y);
        if (geo != null) {
            geo = geo.clone();
            const t = this.graph.view.translate;
            const s = this.graph.view.scale;
            let offset = new Point_js_1.default(t.x * s, t.y * s);
            const parent = this.state.cell.getParent();
            if (parent && parent.isVertex()) {
                const pState = this.graph.view.getState(parent);
                if (pState)
                    offset = new Point_js_1.default(pState.x, pState.y);
            }
            const index = (0, mathUtils_js_1.findNearestSegment)(state, pt.x * s + offset.x, pt.y * s + offset.y);
            if (geo.points == null) {
                geo.points = [pt];
            }
            else {
    ...
    getHandleFillColor(index) {
        ...
        let color = config_js_1.HandleConfig.fillColor;
        if ((terminal != null && !this.graph.isCellDisconnectable(cell, terminal, isSource)) ||
            (terminal == null && !this.graph.isTerminalPointMovable(cell, isSource))) {
            color = Constants_js_1.LOCKED_HANDLE_FILLCOLOR;
        }
        else if (terminal != null &&
            this.graph.isCellDisconnectable(cell, terminal, isSource)) {
            color = config_js_1.EdgeHandlerConfig.connectFillColor;
        }
        return color;
    }
    /**
    ...
    redrawHandles() {
        ...
        this.label = new Point_js_1.default(this.state.absoluteOffset.x, this.state.absoluteOffset.y);
        this.labelShape.bounds = new Rectangle_js_1.default(Math.round(this.label.x - b.width / 2), Math.round(this.label.y - b.height / 2), b.width, b.height);
        ...
        if (this.bends != null && this.bends.length > 0) {
            ...
            this.bends[0].bounds = new Rectangle_js_1.default(Math.floor(x0 - b.width / 2), Math.floor(y0 - b.height / 2), b.width, b.height);
            ...
            this.bends[bn].bounds = new Rectangle_js_1.default(Math.floor(xn - b.width / 2), Math.floor(yn - b.height / 2), b.width, b.height);
        ...
        if (this.virtualBends && this.virtualBends.length > 0) {
            ...
            for (let i = 0; i < this.virtualBends.length; i += 1) {
                if (this.virtualBends[i] != null && this.abspoints[i + 1] != null) {
                    ...
                    if (b.bounds) {
                        b.bounds = new Rectangle_js_1.default(Math.floor(x - b.bounds.width / 2), Math.floor(y - b.bounds.height / 2), b.bounds.width, b.bounds.height);
                        b.redraw();
                    }
                    (0, styleUtils_js_1.setOpacity)(b.node, config_js_1.EdgeHandlerConfig.virtualBendOpacity);
                    last = pt;
                    if (this.manageLabelHandle) {
        ...
        for (let i = 1; i < this.bends.length - 1; i += 1) {
            if (this.bends[i] != null) {
                if (this.abspoints[i] != null) {
                    ...
                    this.bends[i].bounds = new Rectangle_js_1.default(Math.round(x - b.width / 2), Math.round(y - b.height / 2), b.width, b.height);
                    if (this.manageLabelHandle) {
                        this.checkLabelHandle(this.bends[i].bounds);
                    }
                    else if (this.handleImage == null &&
                        this.labelShape.visible &&
                        (0, mathUtils_js_1.intersects)(this.bends[i].bounds, this.labelShape.bounds)) {
                        const w = config_js_1.HandleConfig.size + 3;
                        const h = w;
                        this.bends[i].bounds = new Rectangle_js_1.default(Math.round(x - w / 2), Math.round(y - h / 2), w, h);
                    }
                    this.bends[i].redraw();
                }
                else {
    ...
    checkLabelHandle(b) {
        ...
        if ((0, mathUtils_js_1.intersects)(b, b2)) {
    ...
    drawPreview() {
        try {
            if (this.isLabel) {
                ...
                const bounds = new Rectangle_js_1.default(Math.round(this.label.x - b.width / 2), Math.round(this.label.y - b.height / 2), b.width, b.height);
                if (!b.equals(bounds)) {
                    this.labelShape.bounds = bounds;
                    this.labelShape.redraw();
                }
            }
            if (this.shape != null && !(0, arrayUtils_js_1.equalPoints)(this.shape.points, this.abspoints)) {
                this.shape.apply(this.state);
...
class EdgeHandlerCellMarker extends CellMarker_js_1.default {
    constructor(graph, edgeHandler, validColor = Constants_js_1.DEFAULT_VALID_COLOR, invalidColor = Constants_js_1.DEFAULT_INVALID_COLOR, hotspot = Constants_js_1.DEFAULT_HOTSPOT) {
