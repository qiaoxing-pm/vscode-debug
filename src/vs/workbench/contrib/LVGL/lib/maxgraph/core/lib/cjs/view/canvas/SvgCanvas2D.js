...
const useAbsoluteIds = typeof DOMParser === 'function' &&
    !Client_js_1.default.IS_CHROMEAPP &&
    !Client_js_1.default.IS_EDGE &&
 ...
 */
class SvgCanvas2D extends AbstractCanvas2D_js_1.default {
    ...
    createStyle() {
        ...
        (0, domUtils_js_1.write)(style, `svg{font-family:${Constants_js_1.DEFAULT_FONTFAMILY};font-size:${Constants_js_1.DEFAULT_FONTSIZE};fill:none;stroke-miterlimit:10}`);
        return style;
    }
    /**
    ...
    createElement(tagName, namespace) {
        return this.root?.ownerDocument.createElementNS(namespace || Constants_js_1.NS_SVG, tagName);
    }
    /**
     * Returns the alternate text string for the given foreignObject.
     */
    getAlternateText(fo, x, y, w, h, str, align, valign, wrap, format, overflow, clip, rotation) {
        return !(0, utils_js_1.isNullish)(str) ? this.foAltText : null;
    }
    /**
    ...
    createAlternateContent(fo, x, y, w, h, str, align, valign, wrap, format, overflow, clip, rotation) {
        ...
        const s = this.state;
        if (!(0, utils_js_1.isNullish)(text) && s.fontSize > 0) {
            ...
            (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.BOLD) &&
                alt.setAttribute('font-weight', 'bold');
            (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.ITALIC) &&
            ...
            (0, domUtils_js_1.write)(alt, text);
            return alt;
        }
        return null;
    }
    ...
    addNode(filled, stroked) {
        ...
        if (node) {
            ...
            if (filled && s.fillColor !== Constants_js_1.NONE) {
                this.updateFill();
            }
            else if (!this.styleEnabled) {
                // Workaround for https://bugzilla.mozilla.org/show_bug.cgi?id=814952
                if (node.nodeName === 'ellipse' && Client_js_1.default.IS_FF) {
                    node.setAttribute('fill', 'transparent');
                }
                else {
                    node.setAttribute('fill', Constants_js_1.NONE);
                }
                // Sets the actual filled state for stroke tolerance
                filled = false;
            }
            if (stroked && s.strokeColor !== Constants_js_1.NONE) {
                this.updateStroke();
            }
            else if (!this.styleEnabled) {
                node.setAttribute('stroke', Constants_js_1.NONE);
            }
            ...
            // Enables clicks for nodes inside a link element
                ...
                node.setAttribute('pointer-events', Constants_js_1.NONE);
            }
            // Removes invisible nodes from output if they don't handle events
            if ((node.nodeName !== 'rect' &&
                node.nodeName !== 'path' &&
                node.nodeName !== 'ellipse') ||
                (node.getAttribute('fill') !== Constants_js_1.NONE &&
                    node.getAttribute('fill') !== 'transparent') ||
                node.getAttribute('stroke') !== Constants_js_1.NONE ||
                node.getAttribute('pointer-events') !== Constants_js_1.NONE) {
    ...
    updateFill() {
        ...
        if (s.fillColor !== Constants_js_1.NONE) {
            if (s.gradientColor !== Constants_js_1.NONE) {
    ...
    updateStroke() {
        const s = this.state;
        if (s.strokeColor && s.strokeColor !== Constants_js_1.NONE) {
    ...
    createShadow(node) {
        ...
        if (shadow.getAttribute('fill') !== 'none' &&
            (!Client_js_1.default.IS_FF || shadow.getAttribute('fill') !== 'transparent')) {
            shadow.setAttribute('fill', s.shadowColor);
        }
        if (shadow.getAttribute('stroke') !== 'none' &&
            s.shadowColor &&
            s.shadowColor !== Constants_js_1.NONE) {
    ...
    setLink(link) {
        if (!link) {
            ...
            if (node.setAttributeNS == null || this.root.ownerDocument !== document) {
                ...
                node.setAttributeNS(Constants_js_1.NS_XLINK, 'xlink:href', link);
            }
            this.root.appendChild(node);
            this.root = node;
        }
    ...
    image(x, y, w, h, src, aspect = true, flipH = false, flipV = false) {
        ...
        if (!node.setAttributeNS) {
            ...
            node.setAttributeNS(Constants_js_1.NS_XLINK, 'xlink:href', src);
        }
        if (!aspect) {
    ...
    createDiv(str) {
        ...
        if ((0, domUtils_js_1.isNode)(val)) {
            val = `<div><div>${(0, xmlUtils_js_1.getXml)(val)}</div></div>`;
        }
    ...
    getTextCss() {
        const s = this.state;
        const lh = Constants_js_1.ABSOLUTE_LINE_HEIGHT
            ? `${s.fontSize * Constants_js_1.LINE_HEIGHT}px`
            : Constants_js_1.LINE_HEIGHT * this.lineHeightCorrection;
        ...
        const fontStyle = s.fontStyle;
        (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.BOLD) && (css += 'font-weight: bold; ');
        (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.ITALIC) && (css += 'font-style: italic; ');
        const txtDecor = [];
        (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.UNDERLINE) && txtDecor.push('underline');
        (0, utils_js_1.matchBinaryMask)(fontStyle, Constants_js_1.FONT_STYLE_MASK.STRIKETHROUGH) &&
            txtDecor.push('line-through');
    ...
    plainText(x, y, w, h, str, align, valign, wrap, overflow, clip, rotation = 0, dir) {
        ...
        if (clip && w > 0 && h > 0) {
            ...
            if (!Client_js_1.default.IS_CHROMEAPP &&
                !Client_js_1.default.IS_EDGE &&
        ...
        if (!this.styleEnabled || size !== Constants_js_1.DEFAULT_FONTSIZE) {
        ...
        const lh = Math.round(size * Constants_js_1.LINE_HEIGHT);
        ...
        for (let i = 0; i < lines.length; i += 1) {
            const line = (0, StringUtils_js_1.trim)(lines[i]);
            ...
            if (line) {
                ...
                (0, domUtils_js_1.write)(text, line);
    ...
    updateFont(node) {
        const s = this.state;
        if (s.fontColor && s.fontColor !== Constants_js_1.NONE) {
            node.setAttribute('fill', s.fontColor);
        }
        if (!this.styleEnabled || s.fontFamily !== Constants_js_1.DEFAULT_FONTFAMILY) {
    ...
    addTextBackground(node, str, x, y, w, h, align, valign, overflow) {
        ...
        if (s.fontBackgroundColor != null || s.fontBorderColor != null) {
            ...
            if (overflow === 'fill' || overflow === 'width') {
                ...
                bbox = new Rectangle_js_1.default((x + 1) * s.scale, y * s.scale, (w - 2) * s.scale, (h + 2) * s.scale);
                ...
                try {
                    ...
                    bbox = node.getBBox();
                    bbox = new Rectangle_js_1.default(bbox.x, bbox.y + 1, bbox.width, bbox.height + 0);
                }
                catch (e) {
            ...
            if (bbox == null || bbox.width === 0 || bbox.height === 0) {
                ...
                div.style.lineHeight = Constants_js_1.ABSOLUTE_LINE_HEIGHT
                    ? `${s.fontSize * Constants_js_1.LINE_HEIGHT}px`
                ...
                (0, utils_js_1.matchBinaryMask)(s.fontStyle, Constants_js_1.FONT_STYLE_MASK.BOLD) &&
                    (div.style.fontWeight = 'bold');
                (0, utils_js_1.matchBinaryMask)(s.fontStyle, Constants_js_1.FONT_STYLE_MASK.ITALIC) &&
                    (div.style.fontStyle = 'italic');
                str = (0, StringUtils_js_1.htmlEntities)(str, false);
                ...
                bbox = new Rectangle_js_1.default((x + 1) * s.scale, (y + 2) * s.scale, w * s.scale, (h + 1) * s.scale);
            }
            if (bbox != null) {
                const n = this.createElement('rect');
                ...
                if (this.root?.ownerDocument === document && (0, mathUtils_js_1.mod)(sw, 2) === 1) {
...
SvgCanvas2D.createCss = (w, h, align, valign, wrap, overflow, clip, bg, border, flex, block, scale, callback) => {
    ...
    const pt = (0, styleUtils_js_1.getAlignmentAsPoint)(align, valign);
    ...
    if (wrap && w > 0) {
        block += `white-space: normal; word-wrap: ${Constants_js_1.WORD_WRAP}; `;
        fw = `width: ${Math.round(w)}px; `;
