...
const PolylineShape_js_1 = __importDefault(require("./PolylineShape.js"));
const EdgeMarkerRegistry_js_1 = require("../../style/marker/EdgeMarkerRegistry.js");
/**
 ...
 */
class ConnectorShape extends PolylineShape_js_1.default {
    ...
    createMarker(c, pts, source) {
        ...
        let result = null;
        const n = pts.length;
        const type = (source ? this.style.startArrow : this.style.endArrow) || Constants_js_1.NONE;
        let p0 = source ? pts[1] : pts[n - 2];
        const pe = source ? pts[0] : pts[n - 1];
        if (type !== Constants_js_1.NONE && p0 !== null && pe !== null) {
            let count = 1;
            ...
            const unitX = dx / dist;
            const unitY = dy / dist;
            const size = (source ? this.style.startSize : this.style.endSize) ?? Constants_js_1.DEFAULT_MARKERSIZE;
            // Allow for stroke width in the end point used and the
            // orthogonal vectors describing the direction of the marker
            const filled = (source ? this.style.startFill : this.style.endFill) ?? true;
            result = EdgeMarkerRegistry_js_1.EdgeMarkerRegistry.createMarker(c, this, type, pe, unitX, unitY, size, source, this.strokeWidth, filled);
        }
        return result;
    }
    /**
    ...
    augmentBoundingBox(bbox) {
        ...
        // Adds marker sizes
        let size = 0;
        if (this.style.startArrow !== Constants_js_1.NONE) {
            size = (this.style.startSize ?? Constants_js_1.DEFAULT_MARKERSIZE) + 1;
        }
        if (this.style.endArrow !== Constants_js_1.NONE) {
            size = Math.max(size, this.style.endSize ?? Constants_js_1.DEFAULT_MARKERSIZE) + 1;
        }
        bbox.grow(size * this.scale);
    }
}
exports.default = ConnectorShape;
